...
**2.** Not accurate. Kubernetes is an **orchestrator** — it decides what should run where. A separate **container runtime** on each machine does the work of actually starting the containers [source: k8s-docs-cluster-architecture-2026-08-23]. *[cross-bearing: see Ch 3 §4 — the Container Runtime Interface]*
...
**The Linux Foundation's KCNA exam page does not state a question count or a passing score.** The page states the format, the duration, the eligibility terms, the price, and the domain weights. It does not state how many questions you will see or what percentage you need to answer correctly [source: lf-kcna-exam-page-2026-08-23].

You will nonetheless see "60 questions, 75% to pass" repeated across dozens of study sites, videos, and forum threads, usually stated flatly, as fact. Those figures are widely and consistently reported by third parties. Neither appears on the page the certifying body publishes for this exam [source: lf-kcna-exam-page-2026-08-23].

<!-- AUTHOR-REVIEW: The curriculum-alignment audit reports that the Linux Foundation DOES publish a 75% passing score for multiple-choice exams, in its multiple-choice candidate FAQ (docs.linuxfoundation.org/tc-docs/certification/faq-mc), and that the research stage cached a corrected snapshot which was never written to sources/. No such snapshot exists on disk, so the claim cannot be verified here. This section has been narrowed to what the cached exam-page snapshot supports verbatim ("not stated on this page"), which is true under either reading. IF the FAQ snapshot lands, rewrite this passage to the two-provenance framing: the passing score IS published (75%, in a candidate FAQ most readers never open) while the question count is published nowhere — which sharpens the hazard rather than weakening it. Do not resolve this from memory. -->
...
- **A is wrong.** The question count is not on the exam page. "60 questions" is a third-party report, repeated so consistently that it reads as official. It isn't.
- **B is wrong.** The passing score is not on the exam page either. "75% to pass" travels with the 60-question figure and has the same status: widely reported, not stated where the certifying body describes this exam [source: lf-kcna-exam-page-2026-08-23].
...
> **Logbook Entry:** A colleague of mine sat the KCNA earlier this year having studied hard from a course recorded in 2024.
>
...
- **A is wrong**, and it's the assumption almost every candidate arrives with — the one the Soundings tested and §4 exists to dislodge. The phrase contains the word "cloud," which does most of the misleading on its own.
...
| Not on the exam page | Question count and passing score. "60 questions, 75%" is commonly reported by third parties, not stated where the certifying body describes this exam |
