...
2. You pull `myapp:v2` today. A teammate pulls the same reference next week. Are you guaranteed the same bytes?
...
6. Who defines the container image format, meaning the layout of the artifact you push to a registry and pull onto a machine? A single company, or something else?
...
4. **Once.** Images are assembled from stacked layers, and a shared base is a shared layer. §2 has the structure.
...
- **A is wrong**, and this is the most common misconception about the OCI. It publishes specifications; it does not ship a runtime. runC was *donated to* it [source: oci-overview-2026-08-23], which is a different relationship.
...
This is the fiddliest material in the chapter and the highest value per minute of anything in it. The rules are small, entirely conditional, and almost nobody guesses the defaults correctly.
...
Twenty-five questions. Five require combining two sections; those are marked. Explanations cover every option.
...
> **A** is wrong: the kernel is the shared thing, and that sharing is the definition. **B** is wrong: a hypervisor belongs to the virtualization model, and no per-container hypervisor exists in the container model. **D** is wrong: hardware is the host's, in both models.
...
> **A** is vulnerability scanning, a supply-chain concern handled by other tools entirely [source: k8s-docs-cloud-native-security-2026-08-23]. **C** is invented; no lifecycle phase probes registry capabilities. **D** is wrong: no phase selects a base image by host kernel, since the base images are part of the builder's configured stack.
...
> **A** is wrong and states the conflation as a rule. **C** is wrong: a Kubernetes interface does not supersede industry specifications about artifact formats, since they are not competing. **D** reaches a correct conclusion by an irrelevant route; maturity level [source: cncf-project-maturity-levels-2026-08-23] has no bearing on which specifications a component implements.
...
> **A** is wrong and confuses this case with the `:latest` and no-tag cases. **C** is never a default. **D** is invented: omitting the field is always legal, and a default applies.
