...
3. No. Kubernetes clusters have two kinds of machine with two different sets of software. §2 and §3 name them.
...
**If you got 6+ right:** Skim, with two exceptions. Read §6 at full pace regardless of your score — six later chapters retrieve it by name. And read §4 rather than skimming it; the optionality material scores well on intuition and still catches people on exam day.
...
Systems have shapes, and the shapes have reasons. Kubernetes looks the way it looks because of a specific sequence of problems that the industry solved one at a time, each solution creating the conditions for the next.
...
Read that column on the right again. Notice how many entries describe something that has to *keep being true* rather than something that has to *happen once*. Self-healing isn't an action; it's a standing condition. Hold that thought — it's the shape of §6.
...
The name comes from the Greek word for helmsman or pilot; "K8s" is the numeronym, with eight letters between the K and the s [source: k8s-history-ten-years-2026-08-23]. The brand you're reading did not choose the maritime register to be cute about it — the subject arrived that way.
...
That's the whole official description, and it's terser than it looks. Two words deserve unpacking. *"All cluster data"* means all of it — every object you created, every object the system created, every piece of state the cluster knows about. And *"consistent"* is the property that makes the rest of the architecture possible: every component that reads from etcd through the API server gets the same answer as every other component. Without that, independent components watching shared state would each be acting on a different picture of the world, and nothing else in this chapter would work.
...
That last sentence is the one to hold onto. This component is genuinely optional — not "optional but everyone runs it," but *absent* from an entire class of real clusters, including the one on your laptop. §4 comes back to it.
...
Node components run on every node, maintaining running Pods and providing the Kubernetes runtime environment [source: k8s-docs-components-2026-08-23]. Three of them. That framing sentence is worth keeping — the node's collective job is to *maintain* running Pods, not to start them once and walk away.
...
Note the word *extend*. Addons are not components in the sense that the eight names in §3 are components. They're cluster extensions, and they're implemented using ordinary Kubernetes resources — the same objects you'll be creating yourself from Chapter 4 onward. An addon runs *on* the cluster, using the cluster's own machinery.
...
>
> **"kube-proxy runs on every node, always."** The documentation says node components run on every node, and then marks kube-proxy optional in the same breath. Both are true: *when it runs, it runs on every node* — but it may not run at all, if a network plugin does the same work.
>
...
Every actor in the cluster — the CLI you type into, the scheduler making placement decisions, the controller-manager running its dozen controllers, every kubelet on every node — interacts with the same front end. None of them keeps its own copy of the truth. None of them has a private channel to another component. They all read from and write to one place.
...
That is not a caveat. That is a design position, and it is unusual enough to be worth a moment.
...
- **C is wrong** — and it's the reversal worth catching. Desired state is what you asked for; current state is what's true. Getting them backwards inverts the entire model.
...
**Q3 — B.** Controller A works entirely inside the cluster: it reads desired state from the API server and asks the API server to create or remove Pods — control via the API server. Controller B needs something outside the cluster to set up new Nodes, so it finds desired state from the API server and then communicates directly with an external system — direct control [source: k8s-docs-controllers-2026-08-23].
...
- **C and D are wrong** — invented mechanisms. Worth noting the connection to this chapter, though: the control loop's response to a changed desired state is the same instinct — replace, don't mutate.
...
That's when the control loop stops being an architecture diagram and becomes something you operate. You'll write a description, submit it, and watch a loop you now understand go to work on it — and the reason that will feel unremarkable rather than magical is that you did the work in this chapter first.
