...
**Total time: ~110 minutes | Recommended: split across two sessions (§1–§3, then §4–§6)**

| Section | Time | Attention Cost | Best Time to Study |
|---|---|---|---|
| §1 You File a Declaration | 8 min | Low | Anytime |
...
| §5 The Universal Join | 18 min | Medium | When alert |
| ☆ Taking Your Bearings #3 | 7 min | Medium | After a brief pause |
| §6 A Declaration, Not an Order | 7 min | Low | Anytime |
| Practice Questions | 25 min | High | Peak attention |
...
Before reading this chapter, try these eight questions. Your score determines how to approach the content — no shame in any score, just different reading strategies. Nothing here requires material from this chapter; everything is either general IT knowledge or something Chapters 2 and 3 already gave you.
...
5. **From outside the image** — an environment variable, a mounted config file, a startup argument, or a call to a configuration service. If the difference lived *inside* the image you'd need two images, which defeats the point of having one.
...
7. **A query with a predicate over an attribute** — `WHERE environment = 'production'`, a tag filter, a saved search. The general shape is: things carry attributes, and you name a subset by describing the attributes rather than listing the members.
...
You have spent three chapters learning about a system that runs software. You are now going to spend a chapter learning to write files that describe what *should be true* — and then hand them to something that never receives an instruction. You will not, anywhere in the pages ahead, tell Kubernetes to do anything. You will state what you want to exist. Something else does the doing, and you never address it directly. If you arrived here from imperative operations tooling — from a world of scripts, runbooks, and commands that execute and finish — this is genuinely strange for about a chapter. It is worth naming rather than smoothing over. We will name it now, live with it through §2 to §5, and resolve it in §6.
...
The stakes are structural, and worth stating plainly and once. Every chapter after this one reads *through* this one. Chapter 5's Pod is an object with a spec. Chapter 6's controllers select their workloads by label. Chapter 9's Service selects its backends by label. Chapter 12's role-based access control is, underneath the terminology, a namespaced-versus-cluster-scoped problem wearing a costume. A reader who leaves this chapter able to recite the four required fields but unable to explain what `status` is *for* will re-learn this chapter four more times, in worse conditions, under exam pressure. So take §2 slowly. It is the cheapest hour in the book.
...
The verb you will use to submit a record is `kubectl apply`, which applies a configuration change to a resource from a file or standard input [source: k8s-docs-kubectl-overview-2026-08-23]. That sentence is the entire treatment `apply` gets in this chapter. You cannot teach a record of intent without showing how a record gets filed, so `apply` appears here; the full command surface — its verbs, its flags, its relationship to `create` and `replace`, how it authenticates — belongs to Chapter 8 *[cross-bearing: see Ch 8 — kubectl, in full]*.

<!-- AUTHOR-REVIEW: the object-management taxonomy (imperative commands vs imperative object configuration vs declarative object configuration, and the guidance against mixing techniques on one object) is NOT in the cached source set — kubernetes.io/docs/concepts/overview/working-with-objects/object-management/ was not fetched at Stage 2. This section has been narrowed to the sourced claim per outline Open Question #1. If that page is fetched during revision, the three-technique taxonomy and a 🔭 Closer Look callout should be restored here. Do not draft the taxonomy from memory. -->
...
That is the complete structural vocabulary. Not a starting subset that gets extended for advanced resources — the complete thing. A Pod manifest has those four fields. A NetworkPolicy manifest has those four fields. A custom resource for a database operator that some vendor shipped last week has those four fields. This is why the transferability claim from *Why This Chapter Matters* is not marketing: the structure does not vary by resource type, and it does not expire.
...
`metadata` holds more than name and UID. Two of its other residents — **labels** and **annotations** — are load-bearing enough to get their own section, so they are named here and deferred *[cross-bearing: see Ch 4 §5 — labels, selectors, and annotations]*. Everything else that can appear in `metadata` is above associate tier.
...
*(Deployment is Chapter 6's resource and the instances it manages are Chapter 5's. Use the example here for its shape — a number in a spec, a reality in a status, a correction — and meet the resource properly later. [cross-bearing: see Ch 6 §1 — Deployments and ReplicaSets])*
...
> 🪝 **Snag:** `status` is not something you write. You can type a `status` block into a manifest and apply it; the system will simply overwrite it with what is actually true. It is a report, not a request. Practitioners arriving from imperative tooling try this at least once, usually while trying to make a stuck object look healthy.
...
- **C is right** because the control plane "continually and actively manages every object's actual state to match the desired state you supplied" [source: k8s-docs-objects-2026-08-23]. Continual management implies there are moments when the states differ. That is not breakage; that is the system working. Hold onto this one — Chapter 13 depends on you not having the opposite instinct, because a practitioner who reads every `spec`/`status` gap as a fault will spend a lot of time investigating healthy clusters *[cross-bearing: see Ch 13 §1 — reading status before reading logs]*.
...
That is the whole of the core idea: a namespace is a **scope for names**. Two teams can both have a Service called `database` and neither has to rename anything, because the two names do not collide — they are in different scopes.

The documentation's guidance on when to use them is more restrained than most preparation material admits, and it is worth quoting because the restraint is itself testable. Namespaces are intended for use in environments with many users spread across multiple teams, or projects. For clusters with a few to tens of users, **you should not need to create or think about namespaces at all**. Start using them when you need the features they provide [source: k8s-docs-namespaces-2026-08-23]. Namespaces are not a maturity badge; they are a tool with a specific job.
...
**`kube-public` is a convention, not an enforcement.** The documentation is explicit: *the public aspect of this namespace is only a convention, not a requirement* [source: k8s-docs-namespaces-2026-08-23]. Preparation material routinely states it as a hard property of the namespace. It is not; it is a norm about what people put there.

**`kube-node-lease` is the one people forget**, and it connects forward. Those Lease objects are how the control plane knows a node is still alive; when the heartbeats stop, the node controller marks the node's condition and eventually acts *[cross-bearing: see Ch 8 §4 — node conditions and heartbeats]* [source: k8s-docs-nodes-2026-08-23].
...
You have one container image. It needs to run in development and in production. Chapter 2 established that images are immutable — you do not change a running container's code; you build a new image and recreate the container [source: k8s-docs-containers-2026-08-23]. So if the two environments need different settings, and the image cannot differ, the settings must live somewhere other than the image.
...
**The namespace requirement.** The Pod and the ConfigMap must be in the same namespace [source: k8s-docs-configmap-2026-08-23]. That is §3 collecting rent one section later — a namespace is a scope for names, and a Pod naming a ConfigMap is naming it inside its own scope.
...
> 🪝 **Snag:** You edited the ConfigMap. The running application did not notice. This is the single most common day-one surprise with ConfigMaps, and it is not a bug — three of the four consumption paths are applied by the kubelet *at container launch*. Only the fourth, where your code reads the Kubernetes API itself, subscribes to changes [source: k8s-docs-configmap-2026-08-23]. If you want the first three to pick up a change, something has to cause the containers to be launched again.

**Immutability.** Starting from v1.19 you can add an `immutable` field to a ConfigMap definition. Once a ConfigMap is marked as immutable, **it is not possible to revert this change**, nor to mutate the contents of its `data` or `binaryData` fields. You can only delete and recreate the ConfigMap [source: k8s-docs-configmap-2026-08-23]. Notice the shape of that: it is the same replace-don't-mutate instinct that governs container images, applied to configuration.
...
> ★ **Fixed Point:** **Neither object encrypts anything.** A ConfigMap provides no secrecy or encryption. A Secret is stored unencrypted by default and is readable by anyone with API access, etcd access, or the ability to create a Pod in its namespace. What a Secret adds is *handling*: a distinct object type, a distinct surface for access-control rules, and a defined place to attach encryption at rest. The difference between the two is intent and treatment — not cryptography [source: k8s-docs-configmap-2026-08-23] [source: k8s-docs-secret-2026-08-23].

<!-- AUTHOR-REVIEW: the widely-cited detail that a Secret's `data` field is base64-encoded is NOT supported by the cached source set — k8s-docs-secret-2026-08-23.md is an abridged capture that covers the storage caution and the types table but not the data-field encoding. Per outline Open Question #2 the claim has been omitted rather than asserted. Soundings Q6 pre-tests the base64/encryption distinction as a general-IT prior, which stands on its own. If the full Secret page is re-fetched during revision, restore the base64 clause here. -->
...
>
> The practical consequence is that permission to create Pods in a namespace is, in security terms, a Secrets question — and that is not obvious from the name of the permission. This is exactly the thread Chapter 12 picks up.

### Types of Secret

Secrets carry a type, which signals what kind of data they hold. The built-in types [source: k8s-docs-secret-2026-08-23]:
...
>
> This pair produces more confident wrong answers than anything else in this chapter. Five of them, all in one place:
>
...
>
> **"Pods in `team-a` can reference the ConfigMap in `team-b`."** They cannot. The Pod and the ConfigMap must be in the same namespace [source: k8s-docs-configmap-2026-08-23] — which is just §3's scope-for-names rule showing up where you actually trip over it.

One forward pointer before we move on. "Store config in the environment" is the third of the twelve factors [source: twelve-factor-app-2026-08-23], a methodology that predates Kubernetes and that these two objects implement almost exactly. We will name that connection properly when we look at what cloud native application delivery actually assumes about your application *[cross-bearing: see Ch 15 §2 — the twelve factors, and which ones Kubernetes hands you for free]*. Both objects also appear again as volume types, mounted into a Pod's filesystem *[cross-bearing: see Ch 11 §3 — ConfigMap and Secret volumes]* [source: k8s-docs-volumes-2026-08-23].

---

## ☆ Taking Your Bearings: Scoping and Configuration Objects
...
**4.** 🔵 **[retrieval: ch2]** Chapter 2 named five ways to give a cluster access to a private image registry and deferred the most common one to this chapter. Which Secret type implements it, and what file does it serialize?
...
- **B is the most seductive** of the three because a volume feels live in a way an environment variable does not, and in some configurations the projected file content does eventually change on disk — but the application still has to be written to notice, and a container using a ConfigMap as a `subPath` volume mount will not receive updates at all [source: k8s-docs-volumes-2026-08-23]. Treating "it's a volume, so it's live" as a rule will burn you.

**4. [retrieval: ch2]** The type is **`kubernetes.io/dockerconfigjson`**, and it serializes a `~/.docker/config.json` file [source: k8s-docs-secret-2026-08-23]. It is attached to a Pod through `imagePullSecrets`, which was one of the five credential mechanisms Chapter 2 listed [source: k8s-docs-images-2026-08-23]. If you named `kubernetes.io/dockercfg` instead, that is the older `~/.dockercfg` format — a real type, but not the one in current use.
...
Via a label selector, a client or user can identify a set of objects. The documentation gives it a title, and the title is worth memorizing verbatim: **the label selector is the core grouping primitive in Kubernetes** [source: k8s-docs-labels-selectors-2026-08-23].
...
Write that one down, because you are about to meet it constantly. A ReplicaSet knows which Pods are *its* Pods by selector *[cross-bearing: see Ch 6 §3 — a controller's selector and the Pods it owns]*. A Service identifies the set of Pods behind it by selector, which is what makes Pod churn survivable [source: k8s-docs-service-2026-08-23] *[cross-bearing: see Ch 9 §2 — a Service selects its backends]*. Node scheduling constraints use labels on nodes — the recommended approaches all use label selectors to facilitate the selection [source: k8s-docs-assign-pod-node-2026-08-23] *[cross-bearing: see Ch 7 §2 — node labels and nodeSelector]*. A NetworkPolicy uses a selector to specify what traffic is allowed to and from the Pods that match [source: k8s-docs-network-policies-2026-08-23] *[cross-bearing: see Ch 10 §3 — NetworkPolicy selects both its subject and its peers]*.
...
**Annotations** are the other half of `metadata`'s user-supplied data, and the cleanest way to define them is by contrast: **non-identifying information should be recorded using annotations** [source: k8s-docs-labels-selectors-2026-08-23]. They are non-identifying metadata [source: k8s-docs-objects-2026-08-23].
...
<!-- AUTHOR-REVIEW: annotations have thin cached coverage — the only sourced statements are the labels page's "Non-identifying information should be recorded using annotations" and the objects page's related-concepts listing. Per outline Open Question #3, annotations are taught here purely as the contrast to labels; conventional-use examples (tool metadata, ingress controller configuration, change-cause records) are deliberately omitted rather than asserted unsourced. If working-with-objects/annotations/ is fetched during revision, this subsection can be expanded. -->

> ⚠ **Navigational Hazards**
>
> Two mistakes live here, and they are the same mistake twice.
>
> **Recording something as an annotation and then trying to select on it.** Selectors operate over labels. An annotation cannot be selected on — not because it is a less capable field, but because that is the definition of the two. A controller that needs to find its objects cannot find them by annotation.
>
> **Using a namespace to separate two versions of the same software.** §3 flagged this and deferred the reason. Here it is: namespaces partition *names*. Labels partition *sets*. Nearly everything in Kubernetes operates over sets — every controller, every Service, every policy — so partitioning by namespace when you meant to partition by set means the mechanism that would have grouped your objects cannot see across the line you drew. The documentation's advice to use labels for different versions of the same software is not a stylistic preference [source: k8s-docs-namespaces-2026-08-23]; it is the difference between a distinction the system can act on and one it cannot.
>
> Both errors are the same shape: reaching for the wrong partitioning tool. Learn the shape, not the two facts.

---

## ☆ Taking Your Bearings: Labels, Selectors, and Annotations
...
A) Labels are cluster-scoped, so the second namespace's labels were silently overwritten
...
**3.** The first case is a **label**: a controller needs to *find* objects by it, which is exactly what labels are for and exactly what a selector operates over. The second is an **annotation**: non-identifying information should be recorded using annotations [source: k8s-docs-labels-selectors-2026-08-23]. Note that the *content* is identical in both cases — the same forty-character hash. What determines the answer is not the data, it is whether anything will ever need to select on it.

**4. Answer: B.**
- **A is wrong** on two counts: labels are per-object, not cluster-scoped, and nothing overwrites them.
...
- **B is right**, and it is the item worth carrying forward. Selection is a set operation; namespace is a scope. A query issued inside one namespace considers the objects in that namespace. This combination — a selector plus a namespace boundary — is the exact reasoning Chapter 10 needs for network policy, where the question *which Pods does this apply to?* has to be answered on both sides of a namespace line [source: k8s-docs-network-policies-2026-08-23].
...
✓ Annotations, and the one-sentence rule that separates them from labels
...
It is worth naming what that architecture buys you, because a distinction without a payoff is just trivia.
...
Kubernetes takes imperative instruction all day. `kubectl delete` deletes. `kubectl exec` runs a command inside a container. `kubectl port-forward` forwards a port. None of those has a declarative reading; they are pure imperatives and they are entirely normal to use [source: k8s-docs-kubectl-overview-2026-08-23].

The accurate claim is narrower than the chapter subtitle, and better:

**The objects are declarations, and the imperative commands work by changing declarations.** `kubectl scale` updates the size of a workload [source: k8s-docs-kubectl-overview-2026-08-23] — which is to say, it edits a number in a `spec`, and then something else notices. The imperative command did not scale anything. It amended a record, and the loop did the rest.

That is the same discipline Chapter 3 applied to "nobody is in charge." Both statements are true in the way that matters and false if you press them literally, and knowing which is which is the difference between understanding a system and reciting a slogan about it.
...
3. **Namespaced versus cluster-scoped**, with Nodes, PersistentVolumes, and StorageClasses as the named cluster-scoped examples — and namespace objects themselves as the one candidates miss.
4. **The four initial namespaces** and what each is for. `kube-node-lease` is the forgotten one, and its purpose (Lease objects for node heartbeats) is the detail that gets asked.
5. **Labels versus annotations** — identifying and selectable, versus non-identifying and not.
6. **ConfigMap versus Secret** — a difference of intent and handling, not of encryption.
...
| Confusing labels with annotations | Labels identify and can be selected on; annotations record and cannot [source: k8s-docs-labels-selectors-2026-08-23] |
...
Five of those eight live in §4, which is why that section carries the chapter's densest warning block and its highest attention cost. If you are budgeting revision time, §4 is where it goes.

---

## Practice Questions

Nineteen questions. Three test material from earlier chapters and are tagged as such. Four require two sections at once, because single-section recall is not what this exam is measuring.
...
A) The kubelet receives it and writes the object to disk on the node
B) The kube-scheduler receives it and holds it in its scheduling queue
C) The kube-apiserver receives it and etcd stores it
...
**Q3.** Which of the following is **not** one of the four fields you must set in a manifest?
...
**Q4.** An object's `status` block shows values that do not match what you wrote in that object's `spec`. Who wrote the `status` block?

A) You did, when you applied the manifest
B) The Kubernetes system and its components
C) `kubectl`, when it rendered the object for display
D) Nobody — `status` is computed on read and not stored
...
**Q6.** Which of these is cluster-scoped?
...
**Q7.** Your team wants to run `v2.3` and `v2.4` of the same application side by side in one cluster. What does the Kubernetes documentation direct you to do?

A) Create a namespace per version — this is the documented use of namespaces
B) Keep both in one namespace and distinguish them with labels
C) Create a namespace per version and nest both inside a parent namespace
D) Create one namespace per version, then use a ResourceQuota to link them

**Q8.** What is the `kube-node-lease` namespace for?

A) Holding leases that grant temporary API access to nodes
B) Holding Lease objects associated with each node, which the kubelet uses to send heartbeats
C) Holding the scheduling reservations that bind Pods to nodes
D) Holding objects created by the Kubernetes system

**Q9.** Which statement about namespaces is correct?
...
**Q10.** What does using a Secret instead of a ConfigMap give you?

A) Encryption of the data at rest, enabled by default
B) A distinct object type, a distinct access-control surface, and a defined place to attach encryption at rest — but no encryption by default
C) Encryption in transit between the API server and the kubelet, which ConfigMaps do not get
D) Automatic redaction so that no principal with API access can read the contents

**Q11.** An application reads a setting from an environment variable populated by a ConfigMap. An operator edits the ConfigMap. What does the running application see?

A) The new value immediately
B) The new value after a short kubelet resync interval
C) The old value — the kubelet used the ConfigMap data when it launched the container
D) An error, because the environment variable is now inconsistent with its source

**Q12.** **[retrieval: ch2]** Chapter 2 established that container images are immutable: changing one means building a new image and recreating the container. ConfigMaps gained an analogous property in v1.19. Which statement about it is correct?

A) An immutable ConfigMap can be edited by an administrator but not by an application
B) Immutability can be toggled off if the ConfigMap has no Pods consuming it
C) Once marked immutable, the change cannot be reverted and the data cannot be mutated — you can only delete and recreate the ConfigMap
D) Immutability applies to the `data` field but the `binaryData` field remains editable

**Q13.** You need to make a 4 MB reference data file available to a containerized application. What does the documentation direct you to do?

A) Split it across four ConfigMaps and mount all four
B) Store it in a Secret, which has no size limit
C) Store it in a ConfigMap — the 1 MiB figure is a recommendation, not a limit
D) Use a volume, a separate database, or a file service — ConfigMap data cannot exceed 1 MiB

**Q14.** A Pod in namespace `team-a` needs configuration that already exists in a ConfigMap named `settings` in namespace `team-b`. What is your option?
...
**Q15.** Which of these is a valid **set-based** selector?
...
**Q16.** Given `matchLabels: {app: web}`, which `matchExpressions` entry is exactly equivalent?

A) `{key: app, operator: Equals, values: [web]}`
B) `{key: app, operator: In, values: [web]}`
C) `{key: app, operator: Exists, values: [web]}`
D) `{key: app, operator: In, values: []}`

**Q17.** A controller is written to manage every object carrying a particular attribute. The attribute has been recorded as an annotation. What happens?

A) The controller finds them, but more slowly than if a label had been used
B) The controller cannot select them — selectors operate over labels, and annotations are non-identifying by definition
C) The controller finds them only if the annotation key uses the `kubernetes.io/` prefix
D) The controller finds them only within its own namespace

**Q18.** A selector for `tier=frontend` returns three objects. You know that four objects in the cluster carry that exact label. What is the most likely explanation?

A) One object's label was applied after creation, and post-creation labels are not indexed
B) The fourth object is in a different namespace than the one the query was scoped to
C) The fourth object also carries an annotation, which suppresses label matching
D) Equality-based selectors return a maximum of three results without pagination

**Q19.** You are looking at a workload manifest that contains both a `spec.replicas` field and a `spec.selector.matchLabels` block. Which field determines *which* objects the workload manages, and which determines *how many* it maintains?
...
**Q1. Answer: C.** Every read and write of cluster state goes through the kube-apiserver, and etcd is the consistent, highly-available key-value store used as Kubernetes' backing store for all cluster data [source: k8s-docs-cluster-architecture-2026-08-23].
- **A** confuses the kubelet's role — it runs containers on a node from PodSpecs provided to it; it is not a store of record.
- **B** picks a real control-plane component with the wrong job; the scheduler watches for Pods with no assigned node and selects nodes for them [source: k8s-docs-cluster-architecture-2026-08-23]. It receives nothing from `kubectl`.
...
**Q3. Answer: C.** The four fields to set are `apiVersion`, `kind`, `metadata`, and `spec` [source: k8s-docs-objects-2026-08-23]. `status` is supplied and updated by the Kubernetes system and its components — writing it accomplishes nothing.

**Q4. Answer: B.** `status` describes the current state of the object, supplied and updated by the Kubernetes system and its components [source: k8s-docs-objects-2026-08-23].
- **A** inverts the authorship boundary.
- **C** is wrong: `status` is part of the stored object, not a display-time computation by the client.
- **D** is wrong for the same reason — it is a real, persisted field on the object.
...
**Q6. Answer: C.** Low-level resources such as nodes and persistent volumes are not in any namespace [source: k8s-docs-namespaces-2026-08-23].
- **A** and **D** are among the documentation's own examples of namespaced objects.
- **B** is namespaced too, and pointedly so — ServiceAccounts are bound to a namespace, and every namespace gets a `default` one on creation [source: k8s-docs-service-accounts-2026-08-23].

**Q7. Answer: B.** It is not necessary to use multiple namespaces to separate slightly different resources, such as different versions of the same software; use labels to distinguish resources within the same namespace [source: k8s-docs-namespaces-2026-08-23].
- **A** is the intuitive habit the documentation specifically corrects.
- **C** additionally fails on nesting: namespaces cannot be nested inside one another [source: k8s-docs-namespaces-2026-08-23].
- **D** misuses ResourceQuota, which divides cluster resources between namespaces rather than linking them.

**Q8. Answer: B.** `kube-node-lease` holds Lease objects associated with each node; node leases allow the kubelet to send heartbeats so that the control plane can detect node failure [source: k8s-docs-namespaces-2026-08-23].
- **A** invents an authorization mechanism; leases here are liveness signals, not credentials.
- **C** describes binding, which is the scheduler notifying the API server of its placement decision — a different mechanism entirely.
- **D** is the definition of `kube-system`, and picking it means the four initial namespaces have blurred together. They are worth separating.

**Q9. Answer: C.** Namespaces cannot be nested inside one another, and each Kubernetes resource can only be in one namespace [source: k8s-docs-namespaces-2026-08-23].
- **A** and **B** contradict those two rules directly.
- **D** is the subtle one and it is still wrong: cluster-scoped resources do not belong to every namespace — they belong to none. That distinction matters enormously for permissions, because "in all namespaces" and "in no namespace" imply very different things about who can be granted access *[cross-bearing: see Ch 12 §3]*.

**Q10. Answer: B.** Kubernetes Secrets are, by default, stored unencrypted in the API server's underlying data store; anyone with API access can retrieve or modify one, as can anyone with etcd access or the ability to create a Pod in the namespace [source: k8s-docs-secret-2026-08-23]. What the object type gives you is a place to apply the four recommended protections, not the protections themselves.
- **A** is the misconception this chapter exists to correct.
- **C** is wrong on the premise: API traffic is TLS-protected as a matter of cluster configuration [source: k8s-docs-cloud-native-security-2026-08-23], and that protection is not specific to Secrets.
- **D** contradicts the documented behavior outright.

**Q11. Answer: C.** For the environment-variable path — as for command-and-args and read-only volume file paths — the kubelet uses the data from the ConfigMap when it launches the containers for a Pod [source: k8s-docs-configmap-2026-08-23]. Only the API-reading path subscribes to updates.
- **A** and **B** both assume a propagation mechanism that does not exist for this path.
- **D** imagines a consistency check the platform does not perform; the container simply holds the value it was launched with.

**Q12. Answer: C.** Once a ConfigMap is marked as immutable, it is not possible to revert this change nor to mutate the contents of the `data` or `binaryData` fields; you can only delete and recreate the ConfigMap [source: k8s-docs-configmap-2026-08-23]. Note the parallel to Chapter 2's image rule: the response to "this needs to change" is *make a new one*, not *edit this one* [source: k8s-docs-containers-2026-08-23].
- **A** invents a privilege distinction. Immutability is a property of the object, not of the caller.
- **B** is the most tempting wrong answer because it sounds like a reasonable safety valve. There is no such escape hatch.
- **D** is wrong: both fields are covered.

**Q13. Answer: D.** The data stored in a ConfigMap cannot exceed 1 MiB; for larger settings, consider mounting a volume or using a separate database or file service [source: k8s-docs-configmap-2026-08-23].
- **A** is a workaround for a limit that exists precisely because ConfigMaps are not designed to hold large chunks of data — it fights the design rather than following the guidance.
- **B** is wrong on its premise: a Secret is described as holding *a small amount* of sensitive data [source: k8s-docs-secret-2026-08-23], and is not the escape hatch for size.
- **C** misreads the limit as advisory.

**Q14. Answer: C.** The Pod and the ConfigMap must be in the same namespace [source: k8s-docs-configmap-2026-08-23]. The configuration has to exist in `team-a`.
- **A** invents a cross-namespace reference syntax that the field does not accept.
- **B** borrows the Service DNS form, which addresses Services over the network [source: k8s-docs-namespaces-2026-08-23] — a ConfigMap is not reached over the network by a Pod's config machinery, and this is a good example of two mechanisms that look similar and are not.
...
**Q15. Answer: B.** Set-based selectors use `in`, `notin`, and `exists` [source: k8s-docs-labels-selectors-2026-08-23].
- **A**, **C**, and **D** are all equality-based: `=`, `==`, and `!=` are the equality-based operators. `!=` in particular reads like a set operation and is not one.

**Q16. Answer: B.** `matchLabels` is a map of `{key, value}` pairs equivalent to `matchExpressions` with operator `In` [source: k8s-docs-labels-selectors-2026-08-23].
- **A** uses an operator that does not exist in this vocabulary; the operators are `In`, `NotIn`, `Exists`, and `DoesNotExist`.
- **C** is a real operator with different meaning — `Exists` tests for the key regardless of value, so it would also match `app: api`.
- **D** is `In` with an empty value list, which matches nothing.

**Q17. Answer: B.** Non-identifying information should be recorded using annotations [source: k8s-docs-labels-selectors-2026-08-23]; selection is a label operation, and the label selector is the core grouping primitive [source: k8s-docs-labels-selectors-2026-08-23]. The fix is not to work around the selector — it is to record the attribute as a label, because wanting to select on it is what makes it identifying.
- **A** implies a performance difference where there is a capability difference.
- **C** inverts the prefix rule: `kubernetes.io/` and `k8s.io/` are reserved for core components, not required of user keys.
- **D** describes namespace scoping, which is real but is not what stops the controller here.

**Q18. Answer: B.** Selection operates within the namespace scope of the query; a namespaced query does not cross the boundary [source: k8s-docs-namespaces-2026-08-23].
- **A** contradicts the documented behavior — labels can be attached at creation time and subsequently added and modified at any time, with no difference in how they select [source: k8s-docs-labels-selectors-2026-08-23].
- **C** invents an interaction between annotations and labels. They are independent maps.
- **D** invents a result limit.

**Q19. Answer: B.** `matchLabels` is a selector, and the label selector is the core grouping primitive — it answers *which objects* [source: k8s-docs-labels-selectors-2026-08-23]. `replicas` sits in `spec` and states desired state: how many should exist, which is exactly the documentation's own worked example of a Deployment spec specifying three replicas [source: k8s-docs-objects-2026-08-23].
- **A** inverts the two.
...
| Initial namespaces | `default` (start without creating one), `kube-system` (system objects), `kube-public` (readable by all, by convention), `kube-node-lease` (node heartbeat Leases) |
...
| Secret | Same shape, different intent and treatment. Stored unencrypted by default. Readable by anyone with API access, etcd access, or Pod-creation rights in the namespace |
| `dockerconfigjson` | The Secret type behind `imagePullSecrets` — a serialized `~/.docker/config.json` |
| Label | Key/value identifying attribute. Selectable. Means nothing to the core system, which is why it works everywhere |
...
| Annotation | Non-identifying metadata. Recordable, not selectable. If you might search on it, it's a label |

---

## 🏆 Safe Harbor

**Voyage Progress:** 🗺️ → 🌊 → 🌅 — Chapter 4 of 15 complete. You have crossed from *watching the system* to *writing to it*, which is the largest single step in this book.

You can now read a manifest you have never seen. You know which of its fields you author and which one is a report. You know where its name has to be unique and which resources have no name-scope at all. You know how to describe a set of objects rather than list one. And you know the exact thing a Secret does and does not do, which is a piece of knowledge a surprising number of working practitioners have wrong.
...
There is a second thing worth watching for. A Pod is described in the documentation as *relatively ephemeral rather than durable*, and it is never rescheduled to a different node — if the node it is on dies, it is not moved; it is replaced by a new, near-identical Pod with a different UID [source: k8s-docs-pod-lifecycle-2026-08-23]. Hold that against what you learned in §1. If the thing that runs your application is designed to be replaced rather than repaired, then a declaration that names a specific Pod is a declaration about something disposable — and something else must be holding the intent that survives it.
