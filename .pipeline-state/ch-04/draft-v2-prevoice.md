...
>
> This pair produces more confident wrong answers than anything else in this chapter. Six of them, all in one place:
>
...
**The syntax difference is the sharper version of the rule.** Annotation keys follow exactly the same two-segment rules as label keys — optional DNS-subdomain prefix, required name segment of 63 characters or less — and `kubernetes.io/` and `k8s.io/` are reserved for core components in both cases [source: k8s-docs-annotations-2026-08-24]. The *values* are where they part company:
...
4. **The four initial namespaces** and what each is for. `kube-node-lease` is the forgotten one, and its purpose (Lease objects for node heartbeats) is the detail that gets asked. `kube-public`'s public aspect is a *convention*.
...
Twenty-one questions. Five test material from earlier chapters and are tagged as such. Four require two sections at once, because single-section recall is not what this exam is measuring.
...
- **B is invalid on length:** the name segment must be 63 characters or less [source: k8s-docs-labels-selectors-2026-08-23]. The 253-character allowance applies to the *prefix*, which is a different segment — mixing the two limits is the specific slip this option targets.
