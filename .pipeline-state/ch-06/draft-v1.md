<!-- AUTHOR-REVIEW (pipeline): this draft was reconstructed from `.draft-v1.md.progress.log`, not read from `draft-v1.md`. The drafting stage emitted two assistant messages: the first stopped at `max_tokens` (119,514 chars), the second continued and closed at `end_turn` (3,961 chars). Stdout capture kept only the final message, leaving `draft-v1.md` at 4,050 bytes beginning mid-table-row. The seam between the two blocks is clean (`[source: k8s-docs-` + `job-2026-08-24]`), so concatenating them restores the chapter exactly; that concatenation is what was voiced here. The fix is to accumulate text across ALL assistant messages when `stop_reason == max_tokens`, not to re-draft. This will hit any chapter whose draft exceeds one max_tokens window. -->

# Chapter 6: Fleets, Not Vessels
## *"Nobody sails one Pod"*

**Domain: Kubernetes Fundamentals — competency D1.1, Kubernetes Core Concepts | Authored weight: ~6% of this book**
**Complexity: Mixed | Novelty: Moderate | Prerequisites: Chapters 3, 4, 5**

<!-- AUTHOR-REVIEW: the ~6% figure is this book's authored allocation, not a published CNCF weight. CNCF publishes four domain weights and twelve named competencies with no sub-weights. Match the exact disclosure phrasing used in the metadata lines of Chapters 2–5 before this ships. -->
...
*If you only have 15 minutes: read §1, jump to the decision tree at the close of §7, then work ☆ Taking Your Bearings #3. That is the highest exam-points-per-minute route through this chapter.*
...
Before you read this chapter, work these eight. Your score tells you how to read. No shame attached to any of them; they are just different approaches. Every question here is answerable from Chapters 3 through 5 or from operational experience you already have. None of them require anything this chapter teaches.
...
1. **The count and the recipe.** Something has to know how many copies you wanted and what a copy looks like, or it cannot tell that one is missing or build a replacement.
...
3. **A list is exact but goes stale;** you must maintain it by hand every time membership changes. **A rule stays current** but can silently capture things you did not intend, because you never enumerated what it would match.

4. **Nearly everything.** Hostnames the replica uses to find the primary, the on-disk data each one expects to open, replication position, client connection strings. The two machines are not copies of each other. They hold different roles and different bytes.

5. **Express it as a property of the machine, not as a number.** "One agent per host" rather than "six agents." The count is then whatever the fleet size happens to be, including next Tuesday's.

6. **The web server exiting is a failure; the backup script exiting is success.** For the web server, healthy means still running. For the backup, healthy means finished with a zero exit status and gone.

7. **Desired state and current state.** When they match, the loop does nothing, and keeps watching. Doing nothing is a valid output of a control loop, not a pause in it.

8. **Chapter 5 said the Pod is replaced, not moved:** a new, near-identical Pod with a different UID. What it left unanswered was *who does the replacing.* That is the question this chapter opens on.

**If you got 6 or more right:** skim. Read the ★ Fixed Points, the ⚠ Navigational Hazards blocks, and §4 in full. The two rolling-update bounds and their defaults are the exam-checkable content, and they are easy to get backwards. Then work all three ☆ Taking Your Bearings checkpoints.
...
What actually changes here is what kind of person you are at a terminal. Chapter 5 made you someone who can read what infrastructure is telling you. This chapter makes you someone who states what should be true and hands the system responsibility for it. That is the real behavioral difference between people new to Kubernetes and people who are not. Newcomers reach for the Pod, then for a script that recreates the Pod, then for a cron entry that checks whether the script ran. Practitioners write down the count and the template and go home. If you have ever written one of those scripts, you already know what it costs to maintain.

The stakes, flat: this competency is roughly six percent of the book's authored allocation, and that number understates the chapter twice. First, this is where the book's spine passes through. Chapter 3 introduced the control loop, this chapter instantiates it, and Chapter 15 generalizes it to a loop whose desired state lives in a Git repository. A reader who does not feel the shape here will meet Chapter 15's synthesis as a fifth list to memorize instead of as a recognition. Second, the workload-resource decision (Deployment or StatefulSet or DaemonSet or Job) is exactly the kind of thing a recognition exam asks about in one sentence, and the three misconceptions that surround it can all be defused by one figure and one rule.
...
- **Explain** how a controller knows which Pods belong to it, and what the API does when you get that wrong.
...
>
> You write the count on the Deployment. The Deployment sets it on the ReplicaSet it currently considers current. Both objects carry a `replicas` field [source: k8s-docs-deployment-spec-fields-2026-08-24] [source: k8s-docs-replicaset-2026-08-24], but only one of them is the layer where the count is *acted on*, and that is the ReplicaSet.
...
Everything Chapter 4 taught you about an object's four required fields is unchanged. Everything Chapter 5 taught you about a Pod's `spec` is unchanged. It has simply moved one nesting level down: `.spec.template` is a Pod template with exactly the same schema as a Pod, except that it is nested and carries no `apiVersion` or `kind` of its own [source: k8s-docs-daemonset-2026-08-24]. You do not need that developed. You need it *located*. *[cross-bearing: see Ch 4 §2 — apiVersion, kind, metadata, spec]*
...
One piece of vocabulary you may meet in older material: **ReplicationController**. It is the legacy API for managing workloads that can scale horizontally, superseded by the Deployment and ReplicaSet APIs; a Deployment that configures a ReplicaSet is now the recommended way to set up replication [source: k8s-docs-replicationcontroller-2026-08-24]. ReplicaSets are the successors to ReplicationControllers. The two serve the same purpose and behave similarly, except that a ReplicationController does not support set-based selector requirements [source: k8s-docs-replicaset-2026-08-24]. That is the whole of what you need: recognize the word, know it is superseded, move on.
...
*[cross-bearing: see Ch 3 §6 — the control loop, and why nobody is in charge]*
...
web-7d4b9c6f8-qq81z    1/1     Running   0          6m
...
One sentence is all the HPA gets here. Where its metrics come from and what else can autoscale are later problems. *[cross-bearing: see Ch 13 §4 — metrics-server is what an HPA reads]* *[cross-bearing: see Ch 17 §3 — the autoscaling landscape]*
...
*[cross-bearing: see Ch 9 §1 — this churn is exactly why something needs a stable name]*
...
A ReplicaSet does not track its Pods by name and does not hold a list. It has a `.spec.selector`, a label selector, using the labels that identify potential Pods to acquire [source: k8s-docs-replicaset-2026-08-24]. The machinery is exactly what Chapter 4 gave you: `matchLabels` is a map of key/value pairs equivalent to `matchExpressions` with the operator `In`, and newer resources such as Job, Deployment, ReplicaSet and DaemonSet support set-based requirements through both [source: k8s-docs-labels-selectors-2026-08-23]. Nothing new to learn, one new place to apply it. *[cross-bearing: see Ch 4 §5 — labels and selectors, the universal join]*
...
You got a runaway: create a Pod, fail to see it, notice the count is still short, create another, and so on. And that outcome is so obviously bad that Kubernetes refuses to let you build it. In a ReplicaSet, `.spec.template.metadata.labels` **must** match `spec.selector` or the object will be rejected by the API [source: k8s-docs-replicaset-2026-08-24]. Same for a Deployment: `.spec.selector` must match `.spec.template.metadata.labels`, or it will be rejected by the API [source: k8s-docs-deployment-spec-fields-2026-08-24]. Same for a DaemonSet: configuration with those two not matching is rejected [source: k8s-docs-daemonset-2026-08-24].

This is worth sitting with, because it is nearly the only place in Kubernetes where the API says *no* instead of accepting your intent and reconciling toward it. Everywhere else the system takes what you wrote and works on it. Here it refuses, because the thing you wrote has no reachable steady state at all.

<!-- AUTHOR-REVIEW: the outline (§3, Bearings #1 item 3) assumes a selector/template mismatch produces a live runaway in the cluster. All three cached sources state the API rejects the object outright. Section rewritten to teach the rejection with the runaway as the *reason* for it, and Bearings #1 item 3 adjusted to match. Please confirm this reading before the fact-accuracy audit. -->
...
> **Logbook Entry:** The overlapping-selector failure has a recognizable signature, and it never looks like a configuration mistake, which is what makes it expensive.
>
> Somebody ships a second workload and copies the first one's manifest as a starting point, including its labels. Both controllers are now querying for the same Pods. Neither one is misconfigured in any way the API can see; each one's template agrees with its own selector, which is all the API validates. But each controller is now counting Pods the other one created, seeing a total that keeps moving for reasons it did not cause, and correcting toward a number the other controller immediately disagrees with.
>
> What you observe is Pods appearing and disappearing on no schedule. What you look for, reasonably, is a crash loop or a failing probe or a node under pressure, because those are what flapping usually means. It is none of those. It is two standing orders written about the same crew.
>
...
*[cross-bearing: see Ch 9 §2 — a Service selects its backends with the same mechanism, which is a different controller reading the same labels]*
*[cross-bearing: see Ch 12 §5 — deleting a workload does not delete everything it referenced]*
...
D) Deployment → Node → Pod. The Node holds the count.
...
**5.** 🔵 **[retrieval: ch4]** Chapter 4 listed several things that use label selectors and told you a ReplicaSet was one of them. In Chapter 9 you will meet a Service, which selects Pods by the same mechanism. What would it mean for one Pod to be selected by both a ReplicaSet and a Service at the same time?
...
- **D is wrong** because a Node is not in the ownership chain at all. A Node is where Pods run, not what decides how many exist.
...
✓ That ownership and selection are two mechanisms, not one
...
`.spec.strategy` specifies the strategy used to replace old Pods with new ones. `.spec.strategy.type` can be `Recreate` or `RollingUpdate`, and **`RollingUpdate` is the default** [source: k8s-docs-deployment-2026-08-23]. A rolling update is shaped by two fields, and these are the exam-checkable content of this chapter:
...
<!-- AUTHOR-REVIEW: the outline's worked example (§4, fig02 spec, and Bearings #2 item 1) states the ceiling for 10 replicas at defaults is 12. The cached source specifies maxSurge rounds UP from a percentage, giving ceil(2.5) = 3 and a ceiling of 13. Draft uses 13 throughout, including fig02 and Bearings #2. Please confirm before the outline's number propagates to the Lodestar. -->
...
Put those together and you have the property Chapter 5 promised you. A new Pod that never reports ready never becomes available. A rollout that cannot accumulate available new Pods cannot proceed to remove old ones without breaching the `maxUnavailable` floor. So it doesn't. **The rollout stalls instead of taking down the service.** Readiness probe failures are listed by name among the reasons a Deployment gets stuck trying to deploy its newest ReplicaSet without ever completing [source: k8s-docs-deployment-spec-fields-2026-08-24], alongside image pull errors, insufficient quota, insufficient permissions, limit ranges, and application runtime misconfiguration.
...
That "if and only if" is the whole content of this section, and it is precisely the kind of boundary a recognition exam tests, because the intuitive answer is wrong and the correct answer fits in one clause. The intuitive answer is "any change to the Deployment creates a revision." It doesn't. Change the replica count and you have changed the Deployment, changed the cluster, and changed nothing about the revision history.
...
Rolling back a Deployment is not undoing an edit, and it is not restoring a backup. It is setting the Pod template to a previous revision's value and letting the same rolling update from §4 run in the other direction: same two ReplicaSets, same `maxSurge` and `maxUnavailable`, same readiness gate, opposite direction of travel. The ReplicaSet that holds the old template is still sitting there with zero replicas, which is exactly why the operation is fast and exactly why deleting the revision history removes the ability to do it.

Same loop. Same mechanics. Opposite direction.

*[cross-bearing: see Ch 14 §5 — Helm rollback and Deployment rollback are different mechanisms wearing the same word]*
*[cross-bearing: see Ch 15 §5 — and a third thing, again wearing it]*
...
B) 13 and 8
C) 13 and 7
D) 12 and 7 — 25% applies jointly to the pair

**2.** 🟡 **This one is meant to be hard. If you miss it, you are in the majority, and the miss is worth more to you than a correct guess would have been.** You scale a Deployment from three replicas to six, then run `kubectl rollout history`. How many new revisions do you see, and why?
...
- **D is wrong** because it treats 25% as a single budget shared between the two. They are independent fields bounding independent quantities: one bounds how many Pods exist, the other bounds how many are usable.
...
If you answered "one," you are in good company and you now have the rule in a form you will not lose, because you paid for it. The underlying reason is worth carrying: a revision *is* a ReplicaSet, and a new ReplicaSet is only needed when the kind of Pod changes. Six copies of the same Pod need the same ReplicaSet that three copies needed.
...
✓ The exact rule for what creates a revision, and what a rollback is
...
That identity is concrete, not conceptual. For a StatefulSet with N replicas, each Pod is assigned an integer ordinal unique across the set, from 0 through N−1, and each Pod derives its hostname from the StatefulSet's name and its own ordinal. The pattern is `$(statefulset name)-$(ordinal)`, so a three-replica StatefulSet named `web` produces `web-0`, `web-1` and `web-2` [source: k8s-docs-statefulset-2026-08-24]. The identity sticks to the Pod regardless of which node it is rescheduled onto [source: k8s-docs-statefulset-2026-08-24].
...
The storage half of this (PersistentVolume, PersistentVolumeClaim, StorageClass, access modes, and the provisioning path) is a chapter of its own, and it needs vocabulary this chapter has not built. The documentation itself flags two consequences you will want when you get there: the storage for a given Pod must either be provisioned by a PersistentVolume provisioner based on the requested storage class or pre-provisioned by an administrator, and deleting or scaling down a StatefulSet does *not* delete the associated volumes, deliberately, because data safety is generally more valuable than automatic cleanup [source: k8s-docs-statefulset-2026-08-24].

*[cross-bearing: see Ch 11 §4 — PersistentVolumes, claims, and how a Pod's storage follows its identity]*

One more open thread, smaller: StatefulSets currently require a Headless Service to be responsible for the network identity of the Pods, and you are responsible for creating it [source: k8s-docs-statefulset-2026-08-24]. What a headless Service is belongs with the rest of Services. *[cross-bearing: see Ch 9 §5 — headless Services and stable DNS names]*

*[cross-bearing: see Ch 16 §3 — debugging StatefulSets and their claims]*
...
Read the first clause again, because the whole resource is in it: *as nodes are added, Pods are added to them.* You never revisit the DaemonSet when the cluster grows. The requirement you expressed was never a number.
...
Two of those come back later. Cluster networking plugins ship as DaemonSets *[cross-bearing: see Ch 9 §7 — CNI plugins and how Pod networking gets implemented]*, and node-level log agents are the canonical observability example *[cross-bearing: see Ch 18 §3 — node-level log collection]*.
...
>
> **Name this shape, because you are going to meet it again.** *The object exists but nothing happens without the component.* An Ingress with no ingress controller is the same shape. `kubectl top` with no metrics-server is the same shape. A vertical autoscaler that isn't shipped by default is the same shape. It is not four gotchas. It is one rule with four instances, and the rule is that Kubernetes will happily accept a record of intent that nothing in the cluster is currently able to act on. *[cross-bearing: see Ch 10 §4 — Ingress without an ingress controller]* *[cross-bearing: see Ch 13 §2 — `kubectl top` without metrics-server]*
...
Four sockets, one pattern. Collecting them into a single statement about what kind of system Kubernetes is belongs later, when you have met all four in their own contexts. *[cross-bearing: see Ch 17 §6 — CRI, CNI, CSI and CRDs, resolved into one story]*

> 🔭 **Closer Look:** CRDs are not the only route to a custom API. The other is API-server aggregation, which offers more flexibility at the cost of writing and operating more of the API machinery yourself [source: k8s-docs-custom-resources-2026-08-23]. The published guidance on choosing is straightforward: consider a custom resource or aggregation if your API is declarative, you want `kubectl` and dashboard support, your resources are naturally cluster- or namespace-scoped, and you want to reuse Kubernetes API support features; prefer a stand-alone API if your API doesn't fit the declarative model or you already have a program that serves it [source: k8s-docs-custom-resources-2026-08-23]. For associate-level purposes: CRD is the common path, aggregation is the rarer one, and knowing that both exist is enough.

*[cross-bearing: see Ch 14 §6 — why Helm charts have a `crds/` directory]*
*[cross-bearing: see Ch 15 §3 — a delivery tool that is, structurally, a controller acting on custom resources]*
...
The wrong version of this answer, "yes, disk means StatefulSet," is worth naming because it is the one most people arrive with, and it fails in both directions. It puts unnecessary machinery around stateless services that happen to cache to disk, and it leaves genuinely identity-dependent workloads in Deployments, where a routine Pod replacement quietly returns the wrong data.
...
**High-priority topics, in descending order of likelihood.**

1. **The workload-resource decision.** Which resource for which shape of work. Figure 6.5 is the single highest-value artifact in this chapter; a recognition exam can ask about it in one sentence.
...
**Q3.** 🔵 You submit a Deployment manifest whose `.spec.selector` does not match `.spec.template.metadata.labels`. What happens?

A) The Deployment is created and enters a Pod-creation loop it cannot terminate
B) The API rejects the object
C) Kubernetes copies the template labels into the selector
D) The Deployment is created with zero replicas
...
D) It is a new Pod that inherits the previous Pod's name and UID for continuity
...
**Q7.** 🔵 A Deployment has ten replicas and default strategy settings. What is the maximum total number of Pods (old plus new) that may exist at any instant during a rolling update?
...
C) It preserves the revision history, which `RollingUpdate` discards
...
B) An image-resizing service that writes temporary files while processing
...
**Q17.** 🟡 A StatefulSet's Pods, like a Deployment's, are found by a label selector. Given that, what does the selector *not* tell you about a StatefulSet's Pods?

A) Which namespace they are in
B) Which of them is which — the selector identifies the set, not the identity of members within it
C) Whether they are running
D) How many there should be
...
A) A namespace annotation on the CRD
...
A) Nothing structural; custom controllers use a different API and run as privileged processes
B) Both follow the same control loop — read `.spec`, act, write `.status` — but built-in controllers run in the control plane while a custom controller normally runs as an ordinary workload, typically a Deployment
C) Both run inside the API server; the difference is only which resources they watch
D) Both run on every node as DaemonSets
...
**Q3 — B.** `.spec.selector` must match `.spec.template.metadata.labels`, or the object will be rejected by the API [source: k8s-docs-deployment-spec-fields-2026-08-24]. **A** describes what *would* happen if the object were accepted, which is precisely why it isn't: a good intuition landing on the wrong answer. **C** is wrong because Kubernetes does not silently repair intent. **D** is wrong because a selector matching nothing is not an error state; the controller would create Pods, not sit still.
...
**Q7 — C, 13.** `maxSurge` defaults to 25%, and the absolute number is calculated from the percentage by **rounding up** [source: k8s-docs-deployment-spec-fields-2026-08-24]. 25% of 10 is 2.5, rounded up to 3. Ceiling = 10 + 3 = 13. **B** is the answer you get if you round down, which is the rule for `maxUnavailable`, not `maxSurge`; this is the single most instructive wrong answer in the set. **A** would mean no surge at all, which is what `maxSurge: 0` gives you (legal only if `maxUnavailable` is non-zero). **D** would be 50%.
...
**Q13 — C.** Members that address each other by stable hostname and own distinct data are, by definition, not interchangeable, which is the criterion [source: k8s-docs-statefulset-2026-08-24]. **A** and **B** are both distractors built from the disk misconception: both write to disk, and both are perfectly happy in a Deployment because replacing one Pod with a differently-named one breaks nothing. **D** writes durable output but holds no identity; the importer could run as any Pod, or as a Job, and nothing downstream would notice.
...
**Q16 — B.** `Succeeded` means all containers in the Pod have terminated in success and will not be restarted [source: k8s-docs-pod-lifecycle-2026-08-23]. **A** confuses in-progress with done. **C** invents a phase; there are five, and `Completed` is not among them [source: k8s-docs-pod-lifecycle-2026-08-23], though you will see the word in `kubectl get pods` output as a container-state reason, which is exactly why the distractor works. **D** is the failure phase: all containers terminated and at least one terminated in failure [source: k8s-docs-pod-lifecycle-2026-08-23].

**Q17 — B.** A selector identifies a set. It answers "is this Pod one of yours"; it cannot answer "which one is this." A StatefulSet's Pods carry identity through their ordinal and derived hostname, `$(statefulset name)-$(ordinal)`, giving `web-0`, `web-1`, `web-2`, and that identity sticks to the Pod across rescheduling [source: k8s-docs-statefulset-2026-08-24]. The identity is a *separate* mechanism layered over selection, which is why the StatefulSet controller also adds a per-Pod name label so you can attach a Service to one specific member [source: k8s-docs-statefulset-2026-08-24]. **A** is wrong; namespace scoping is inherent to the query. **C** is wrong: readiness is on the Pod's status, which the selector leads you to. **D** is wrong: the desired count is `.spec.replicas`, which is a different field but is not something the selector was ever going to tell you either.

**Q18 — B.** On their own, custom resources let you store and retrieve structured data; combining a custom resource with a custom controller is what provides a true declarative API [source: k8s-docs-custom-resources-2026-08-23]. **A** invents a requirement. **C** is backwards: a CRD is the alternative to API aggregation, and defining a CRD means the Kubernetes API serves and stores your resource without you writing an API server [source: k8s-docs-custom-resources-2026-08-23]. **D** is a real feature attached to a false claim; nothing about a `status` subresource is what makes something act on an object. A controller is.

**Q19 — B.** Controllers are client programs that read and/or write the Kubernetes API, following a control loop: read an object's `.spec`, possibly do things, then update the object's `.status` [source: k8s-docs-extending-kubernetes-2026-08-23]. That description covers both. The difference is deployment, not architecture: the operator's controller normally runs outside the control plane, much as you would run any containerized application, for example as a Deployment [source: k8s-docs-operator-pattern-2026-08-23], while the built-in controllers ship with the control plane. **A** is wrong on both counts: operators are ordinary clients of the Kubernetes API [source: k8s-docs-operator-pattern-2026-08-23], not privileged processes. **C** is wrong; controllers talk *to* the API server, and a built-in controller sends messages to the API server that have useful side effects [source: k8s-docs-controllers-2026-08-23] rather than running inside it. **D** invents a deployment model that would make no sense for either.
...
| **`maxSurge` / `maxUnavailable`** | Both default to 25%. Surge is a ceiling on total, rounding up. Unavailable is a floor on available, rounding down [source: k8s-docs-deployment-spec-fields-2026-08-24]. |
...
| **Rollback** | Restore a previous template and run the same rolling update backwards. Ten old ReplicaSets retained by default [source: k8s-docs-deployment-spec-fields-2026-08-24]. |
...
| **DaemonSet** | One Pod per eligible node, added as nodes join [source: k8s-docs-daemonset-2026-08-24]. Not a replica count. |
| **Job / CronJob** | Job runs to completion once [source: k8s-docs-job-2026-08-24]. CronJob creates Jobs on a repeating schedule [source: k8s-docs-cronjob-2026-08-24]. |
