...
<!-- AUTHOR-REVIEW: the KCNA curriculum snapshot is not in this chapter's cached source set (the corpus note lists only the 18 snapshots referenced by this chapter's pipeline-state files). The house metadata form used by Chapters 2, 5 and 7 carries two inline CNCF source tags on this line. Restore them from the book-level curriculum snapshot during revision rather than citing blind. -->
...
**On the session split:** the recommended break after Bearings #2 puts the request path and the node in one session, and versions, disaster and synthesis in the other. That also isolates §6 — the densest pure-recall block in this book — at the start of a fresh session, which is where it belongs.
...
Before reading this chapter, try these eight questions. Your score determines how to approach the content — there is no shame in any score, only different reading strategies. Several of these ask you to reason rather than to recall. Answer in your own words; a one-sentence answer is enough.

1. You have a command-line tool on your laptop that manages a server somewhere else. Before it can do anything at all, what does it need to know — and what changes about that when you become responsible for two of those servers instead of one?
...
5. **Nothing.** `NoSchedule` governs new placements only — Pods already running on the node keep running. Which raises an obvious follow-up question that §4 answers.
...
7. **A newer client talking to an older server is the more dangerous direction** — the client can ask for things the server has never heard of. This intuition is correct in general and, for exactly one component in Kubernetes, wrong. §6 names it.
...
Three words, and a machine goes out of service without a single running process being disturbed. Chapter 7 ended by naming that command, telling you it was this chapter's opening move, and declining to explain it. Here it is.

Look at its shape for a moment, though, because the shape is the more interesting thing. You have been typing commands in exactly this form since Chapter 4 — `kubectl apply -f`, `kubectl get pods`, `kubectl scale` — and nobody has told you what the form *is*. It worked anyway. That is precisely the condition under which a candidate walks into an exam room confident and then loses five points to a question about which component is permitted to be one minor version newer than which.

Chapters 2 through 7 made you someone who can describe what should run and where. This chapter makes you someone responsible for the thing it runs on. That is a different posture, and the vocabulary shifts with it: practitioners on watch think in terms of what they can take out of service safely, what they can stop other people doing, and what they cannot get back. You are about to acquire all three.
...
The stakes, stated plainly: about five points on this book's allocation, which is not many. What the number understates is the *shape* of those points. §6's version-skew material is the single most mechanically checkable block in the entire curriculum — the rules are exact, the numbers are small, and there is no partial credit for nearly remembering them. It is therefore the easiest place in the exam to lose points you had every opportunity to keep. That is the whole of the case for reading this chapter carefully. There is no need to inflate it.
...
`kubectl apply -f deployment.yaml`. `kubectl get pods`. `kubectl scale deployment/web --replicas=5`. Every one of them worked. Nobody told you what the shape was, and you did not need to be told, which is exactly why it is worth ten minutes now.
...
Now put four commands you have already run through those same four slots and watch the shape appear retroactively.
...
**Figure 8.1 —** Four commands you already know, aligned on the same four slots. What to notice is the empty columns: `kubectl get pods` omits NAME and flags, and gets every Pod as a result. What to notice second is the asymmetry at the bottom, which is the examinable half of this section.

That asymmetry is worth stating on its own, because it is precisely the shape of an exam distractor. Resource types are case-insensitive, and you may use the singular, plural, or abbreviated form — `pod`, `pods` and `po` are the same thing [source: k8s-docs-kubectl-overview-2026-08-23]. Resource *names* are case-sensitive [source: k8s-docs-kubectl-overview-2026-08-23]. The tool is relaxed about what kind of thing you meant and exacting about which one.
...
| Verb | What it does | Where you met it |
|---|---|---|
...
| `logs` | Print the logs for a container in a Pod | Ch 13 |
| `exec` | Execute a command against a container in a Pod | Ch 13 |
...
One entry deserves a sentence of its own. `explain` returns documentation for a resource type [source: k8s-docs-kubectl-overview-2026-08-23] — which makes it the only verb in the table that answers a question about *the API* rather than a question about *your cluster*.

> ⚓ **Worth Securing:** `kubectl explain` is the entry in this table that pays off longest. Because it queries the API's own documentation, it works on resource types you have never seen — including Custom Resource Definitions this book does not cover, installed by tools that did not exist when this book was written. Two years from now it will still be the fastest way to find out what a field does.
...
Verb, resource type, name. The grammar, instantiated — and the thing Chapter 7 promised would be Chapter 8's opening move *[cross-bearing: see Ch 7 §4 — the built-in taint node.kubernetes.io/unschedulable]*. What it does is what Chapter 7 already told you: it marks a node unschedulable, preventing the scheduler from placing new Pods onto that Node, without affecting the existing Pods on the Node [source: k8s-docs-nodes-2026-08-23].

That is all this section will say about it. Everything else — what it writes, what it does *not* do, what the second command is, and what happens if you skip that second command — is §4's *[cross-bearing: see Ch 8 §4 — taking a node out of service]*. Carry the question with you; three sections from now it will have a better answer than it would have here.

---

## 🔵 §2 — Three Gates and a Logbook

Soundings question 3 asked you to list the distinct questions a server must answer before doing expensive work. If you produced two — *who are you*, and *are you allowed* — you produced the answer nearly everyone produces, and you have just located a hole in your own model. That is a good place to start reading from, and it is worth naming rather than skipping past: a reader who has just discovered their model is incomplete is the most receptive reader this chapter gets.

There are three.

Chapter 3 named them in passing, at the point the API server was introduced, and pointed here for the treatment *[cross-bearing: see Ch 3 §5 — the API server and what happens at its door]*. The Kubernetes documentation's own guidance on securing a cluster lists them in this order — Controlling Access to the Kubernetes API, Authenticating, Authorization, Using Admission Controllers, and Auditing [source: k8s-docs-cluster-administration-2026-08-23] — and the project's extension-point taxonomy names the same three as the API access extensions: authentication, authorization, and dynamic admission control via webhooks [source: k8s-docs-extending-kubernetes-2026-08-23].

<!-- AUTHOR-REVIEW: the three names and their canonical order are sourced (cluster-administration and extending-kubernetes, both above). What is NOT sourced in any cached snapshot is the sequential-gate semantics this section is built on: that a request passes the three IN ORDER, that admission runs AFTER authorization and BEFORE persistence, and that admission controllers may MUTATE a request rather than only accept or reject it. All three claims are load-bearing for this section's Fixed Point, for ch08-fig02, and for Chapter 12's derivation of Pod Security Admission. Fetch kubernetes.io/docs/concepts/security/controlling-access/ — it closes all of them outright. Secondary if cheap: .../reference/access-authn-authz/admission-controllers/ for the mutating/validating distinction. Do not ship the section's ordering and mutation claims untagged. -->
...
RBAC is one authorizer among several, and it is Chapter 12's material in full *[cross-bearing: see Ch 12 — RBAC: Roles, ClusterRoles, and bindings]*. For now, one fact about it is enough: it is a mechanism that lives at this gate, and it has opinions about identities and verbs, not about the contents of your YAML.
...
<!-- AUTHOR-REVIEW: no cached source states that ResourceQuota and LimitRange are enforced by admission controllers. The claim is standard and near-certain, but it is unsourced here. The controlling-access/ fetch flagged above, or the admission-controllers reference page, should close it. -->
...
<!-- AUTHOR-REVIEW: deliberately minimal. No cached source defines what an audit record contains, what stages are recorded, or that auditing is policy-driven. Per the outline's Open Question #4 the recommendation was option (b) — name it, assert no definition. If Stage 2 fetches kubernetes.io/docs/tasks/debug/debug-cluster/audit/ (same doc tree as the two blocking fetches), upgrade this to two or three sentences during revision. -->
...
<!-- AUTHOR-REVIEW: BLOCKING GAP. The cached sources support exactly two claims — that quota is the mechanism by which namespaces divide cluster resources among users (namespaces snapshot), and the functional contrast in the one cloud-native-security sentence quoted above. NOT cached and therefore NOT asserted below: what a ResourceQuota can count (compute totals, object counts, storage); the rule that once a quota exists in a namespace, Pods must specify requests and limits; LimitRange's min/max/default structure. This section is deliberately shorter than the two published cross-bearings pointing at it deserve. Fetch kubernetes.io/docs/concepts/policy/resource-quotas/ and .../limit-ranges/. Scope guard: take quota scope and what it counts, the requests-must-be-specified rule, and LimitRange's default/min/max shape. Do NOT take quota scope-selectors, priority-class quota, or the full countable-resource list — all above associate tier. -->
...
> ⚓ **Worth Securing:** The two are usually deployed together, and for a reason worth understanding rather than memorising. A quota with no LimitRange means a single team member who omits resource requests can, with one Pod, consume the namespace's entire allocation — because nothing forced them to declare what they intended to use. The quota sets the ceiling; the LimitRange makes sure everyone's consumption is legible to it.
...
**1.** `kubectl` / `describe` (command) / `node` (TYPE) / `worker-3` (NAME); there are no flags. **You could capitalise the TYPE freely** — resource types are case-insensitive and accept singular, plural or abbreviated forms. You could not capitalise the name; names are case-sensitive.
...
Read the middle clause of that first sentence again, because it is doing more work than it looks like it is doing. `cordon` **deliberately leaves the running Pods alone.** That is not an oversight in the tool; it is the point of having a separate step. And it means the phrase "take a node out of service," which sounds like one action, is two.
...
Note the parameter named in the definition: `node-monitor-grace-period`. That is the window. This book will not tell you how long it is, because the cached documentation names the parameter without stating a value, and a number you half-remember is worse than a parameter name you can look up.
...
'Allocatable' on a Kubernetes node is defined as the amount of compute resources that are available for pods [source: k8s-docs-node-allocatable-2026-08-24]. The scheduler treats 'Allocatable' as the available capacity for Pods, and does not over-subscribe it [source: k8s-docs-node-allocatable-2026-08-24]. Capacity is the machine's total; Allocatable is what is left for your workloads after the node's own overheads are set aside. The two differ because some of the machine has to be reserved for the software that makes it a node at all — the kubelet, the container runtime, and the operating system underneath both.

<!-- AUTHOR-REVIEW: the sentence above beginning "The two differ because" is NOT sourced. The node-allocatable snapshot's extraction note is explicit that the Capacity → Allocatable relationship appears only as an image (node-capacity.svg) with no text equivalent, and that kube-reserved, system-reserved, eviction-threshold and cgroup enforcement were deliberately not extracted. Per outline Open Question #6 the recommendation was option (b) — pay the Chapter 7 promise partially by naming what the reservation is FOR without the arithmetic. That is what the sentence does, but it needs either a source or removal. Fetch kubernetes.io/docs/tasks/administer-cluster/reserve-compute-resources/ if the promise is to be paid properly; otherwise cut to the two verbatim-sourced sentences and note in Ch 7's revision that the pointer is chapter-scoped rather than section-pinned. Do NOT state an arithmetic relationship in any case. -->
...
Five questions, and the honest observation is that in most working lives the answers arrive already decided — by budget, by a compliance requirement, by what the platform team standardised on two years ago. Knowing the axes is still worth something, because it tells you what was traded away.
...
That is the general shape of the answer to Soundings question 8. A managed control plane means the provider decides when you upgrade, and the provider holds the etcd backup. A self-hosted control plane means both are yours.
...
**1.** 🔵 **[retrieval: ch7]** Chapter 7 listed `node.kubernetes.io/unschedulable` with a `NoSchedule` effect among the built-in condition taints, and noted that this one is applied deliberately rather than by a failing component. What command applies it — and, using Chapter 7's rule about what `NoSchedule` governs, what happens to the Pods already running on that node?
...
**4.** 🔵 **[retrieval: ch2]** You bootstrap a cluster with kubeadm. Before any node in it can run a Pod, one piece of software must already be installed on that node which kubeadm does not provide. Name it, name the two implementations the documentation names, and say which interface Kubernetes uses to talk to it.
...
**1.** `kubectl cordon`. **Nothing happens to the running Pods** — `NoSchedule` governs new placements only, so Pods already on the node keep running.

The point of the question is the identity rather than the definition: `kubectl cordon` is not a special-purpose maintenance channel. It is Chapter 7's built-in taint, applied by an operator's hand instead of by a failing disk, and the scheduler then behaves exactly as Chapter 7 said it behaves.
...
*Common wrong turns:* `False` (the intuitive answer, and the wrong one) and `NotReady` (which is how it is often displayed in summary output, but is not one of the three condition values).

**4.** A **container runtime** — **containerd** or **CRI-O** — and Kubernetes talks to it through the **CRI**, the Container Runtime Interface.

Six chapters back, Chapter 2 drew this as an interface boundary and it was, at the time, an architectural point. This is the moment it becomes an operational one: the officially supported bootstrapper will build you a control plane and will not install the thing that runs containers, because that lives on the other side of the line.

**5.** The first engineer owns their control plane (self-hosted); the second is on a managed platform where the provider holds the upgrade calendar. Further duties that differ: **etcd backup** (the standard second answer), certificate rotation, control-plane machine maintenance, and control-plane component configuration such as scheduler profiles.
...
✓ What moves when the control plane is somebody else's
...
**Figure 8.5 —** The double line at 0 is the API server's minor version. For every component but one it is a wall: bars extend to the left and stop dead. `kubectl` is the single bar that crosses to the right. The axis is deliberately relative — the rules do not change when the version numbers do.
...
There is a reason, and the reason is worth more than the fact. `kubectl` is a **user tool that lives outside the cluster**, not a component running inside it. It is the thing on your laptop. It is not participating in the cluster's internal consistency, so its compatibility window is about human convenience — you should be able to run one `kubectl` against a fleet of clusters that are not all on the same release — which is why it is the only symmetric window in the table.
...
The order falls out of the rule and does not need memorising separately. If nothing may be newer than the API server, then the API server must be upgraded first; everything else follows behind it, within its permitted window.
...
> ⚓ **Worth Securing:** "Store it outside the control plane nodes" is the entire point of that sentence, and it is the half people skip. A snapshot that lives only on the machines it exists to protect you against losing is not a backup. It is a copy that dies with the original.
...
And whose job is this? §5's answer applies: on a managed control plane you may not be able to reach etcd at all, and the provider holds this. On a self-hosted cluster it is yours — and it is the one duty in this chapter where doing it late is not the same as doing it.
...
**2.** 🟡 State the one rule that generates four of the five rows of the skew table. Then name the row it does *not* generate, and say why that row is different.
...
**2.** The rule: **nothing may be newer than the API server.** It generates the kubelet row, the kube-proxy row, the controller-manager/scheduler/cloud-controller-manager row, and the constraint on HA API servers relative to one another.

The row it does not generate is **`kubectl`**, and the reason is what makes it memorable: `kubectl` is a user tool living outside the cluster, not a component running inside it. It is not participating in the cluster's internal consistency, so its window is set by human convenience — one release either way, so that one installation can address clusters at slightly different versions.

If you could answer this, you derived the table rather than memorising it, and that is the version of this knowledge that survives to exam day.
...
**0–2:** Re-read §6 with the derivation in mind: one rule, four rows, one exception. Do not try to memorise five rows. Then re-read §7, which is five minutes and has no numbers in it at all.

**Checkpoint: you've now mastered**
✓ The rule that generates the skew table, and the one exception to it
...
Here is the claim I asked you to keep score on.
...
Chapter 7 closed by saying this chapter is where the rules turn into consequences. That is the sentence worth landing on.
...
10. **All objects live in etcd, and access to etcd is equivalent to root permission in the cluster.**
...
| "An etcd snapshot on the control-plane node is a backup" | It is a copy that dies with the thing it was protecting — and, unencrypted, a root-equivalent credential |

---

## Practice Questions

Seventeen questions. Several require two sections at once; that is deliberate, because the exam does not organise itself by chapter section either. Answers and full explanations follow the set.
...
B) Authentication → admission → authorization; only authorization can modify
...
A) `kubectl drain node worker-3`, then `kubectl cordon node worker-3`
...
D) minikube is the officially supported tool for creating clusters; kubeadm installs a container runtime on each node

**13.** Two teams run the same application. Team A uses a cloud provider's managed Kubernetes service; Team B self-hosts on their own hardware. Which set of duties belongs to Team B and not to Team A?

A) Writing Deployment manifests, and choosing container images
B) Deciding when the control plane is upgraded, and taking etcd backups
C) Setting resource requests and limits, and defining namespaces
D) Nothing differs; the two teams have identical operational responsibilities
...
**16.** An operations team stores its nightly etcd snapshots, unencrypted, on the primary control-plane node. Which statement best describes the problem?
...
**17.** You encounter a Kubernetes administrative feature this book has not covered. Based on §8's synthesis, what are the two most productive questions to ask about it first?
...
**2. C.**
Authentication, then authorization, then admission; only admission can modify. **A is wrong** on order and on the modifying gate — authentication establishes identity and has no view on the request's contents at all. **B is wrong** on order; admission runs after authorization, not before it. **D** gets the order right and then gives all three gates a power only the third has, which is the error that collapses three distinct gates into one undifferentiated "security check."
...
**8. B.**
`cordon` first to stop new arrivals, then `drain` to evict what is already there. Note the grammar: `cordon` and `drain` take the node's name directly, without a preceding TYPE, because the verb already implies the resource type. **A reverses the order**, which would drain a node that is still accepting new Pods — the scheduler may place work onto it while you are clearing it. **C is the chapter's headline trap**: `cordon` does not empty anything. **D begins by making the node schedulable again**, which is the opposite of the instruction.

**9. C.**
`Ready` becomes `Unknown` when the node controller has not heard from the node within the `node-monitor-grace-period`. **A is the intuitive wrong answer** and misstates the evidence: `False` means the node *told you* it is unhealthy, which requires the node to be talking. **B is wrong** — `NotReady` is a display convention in summary output, not one of the condition's three values. **D is wrong** and asserts a diagnosis the control plane deliberately declines to make; the whole value of `Unknown` is that it does not claim to know why.

**10. B. [retrieval: ch4]**
`cordon` marks the node unschedulable — a statement of desired state, which lives in `spec`. Chapter 4's rule is the giveaway: `status` is written by the system to report what is observed, and you do not write it. **A is wrong** for that reason: unschedulability is not an observation about the machine, it is an instruction about it. **C is wrong**; the mechanism is a taint and an unschedulable marker, not a label. **D is wrong** and is worth rejecting emphatically, because it is precisely the belief §8 exists to dismantle — there are no side channels.
...
**12. B.**
kubeadm is the officially supported tool for creating clusters, installing the control plane and joining nodes; kind and minikube are the documented local learning tools. **A swaps the two categories.** **C swaps kubeadm and k3s** — k3s is a lightweight distribution, not the official bootstrapper. **D is wrong twice**: minikube is not the production bootstrapper, and kubeadm does not install a container runtime — a runtime must already be present on every node, which is the Chapter 2 boundary showing up as an operational requirement.

**13. B.**
Upgrade timing and etcd backup are the two duties that move when the control plane does. **A is wrong** — both teams write their own manifests and choose their own images regardless of who runs the control plane. **C is wrong** for the same reason; resource requests and namespaces are workload-side concerns. **D is wrong**, and is the answer of someone who has not yet asked whose calendar the upgrade goes on.
...
You have spent seven chapters putting workloads onto machines and one chapter standing over the result, and in all of it, nothing has had to *reach* anything else. The Pods were placed. The nodes were managed. The quotas were enforced. Not once did we ask how a Pod on `worker-3` finds a Pod on `worker-11`, or what address either of them has, or what happens to that address when the Pod is rescheduled somewhere else forty seconds from now — which, given everything Chapter 6 taught you about controllers, it very well might be.

That last part is the difficulty, and it is worth feeling the shape of it before Chapter 9 answers it. Every Pod gets an address. Pods are also, by design, disposable — a controller may replace one at any moment, and the replacement is a different Pod. So the cluster contains a large number of network endpoints, each perfectly valid, none of them stable, and applications that need to talk to each other anyway.
