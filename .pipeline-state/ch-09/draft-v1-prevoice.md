...
**Recommended session split:** stop after ☆ Taking Your Bearings #2. That gives you the model, the abstraction, and the backends in one sitting; the implementation, the names, and the synthesis in the next. It also puts §3 and §7 — the chapter's two densest recall blocks — in different sessions, which matters more than it sounds like it should.
...
> *"A name is a promise that survives the thing it named."*
> — Lodestar Ledgers
...
3. **The client breaks** — it holds a valid-looking address for something that no longer exists. It would not care if it had been given something that doesn't move: a name or an address belonging to the *set* rather than to any member of it. *(Chapter 6 §4.)*

4. **A selector — a query over the same labels.** Editing a Pod's labels can remove it from one controller's set, the other's, or both, independently. The two controllers do not coordinate; they are separate queries that happen to read the same field. *(Chapter 4 §7, Chapter 6.)*
...
Every Pod in the cluster gets its own IP address, unique across the whole cluster. Any Pod can reach any other Pod at that address directly — no NAT, no proxy, no port mapping, whether the two are on the same machine or on machines in different racks [source: k8s-docs-network-model-2026-08-23]. Chapter 8 pointedly did not tell you that. It had reason not to: the claim wasn't sourceable in that chapter's material, and it belongs here, where it can be stated properly and then immediately complicated.

Because here is the complication. That address is worth almost nothing. Chapter 6 taught you that a controller may replace any Pod at any moment, and that the replacement is not a repaired version of the old Pod — it is a *different Pod*, with a different identity and, now that you know §1's rule, a different address. So you have a system that gives every workload a perfectly good address and then guarantees that the address will expire. **The whole subject of this chapter is the gap between those two sentences.**
...
> 🪝 **Snag:** "Each container gets its own IP" is the most common form of this mistake, and it is usually the residue of experience with plain Docker, where something like it is true. Inside a Pod, the address belongs to the **Pod**. Every container in a Pod shares the network namespace, including the IP address and network ports; inside a Pod, and *only* then, the containers can communicate using `localhost` [source: k8s-docs-pods-2026-08-24]. Containers in *different* Pods have distinct IP addresses and must use IP networking to reach each other [source: k8s-docs-pods-2026-08-24].
...
### Kubernetes implements none of this

Everything above is a specification. It describes what must be true, and says nothing about how.

Pod networking is implemented by a **network plugin**, and the interface it plugs into is **CNI, the Container Network Interface** — listed among Kubernetes' infrastructure extension points alongside CSI for storage, CRI for container runtimes, and device plugins for custom hardware [source: k8s-docs-extending-kubernetes-2026-08-23]. CNI plugins are **binary plugins**: external programs that the kubelet executes [source: k8s-docs-extending-kubernetes-2026-08-23].

*[cross-bearing: see Ch 2 §4 — CRI, CNI, CSI and device plugins as Kubernetes' four pluggable interfaces]*. That section named CNI and pointed you here. This is the second of the four, and Chapter 17 collects all of them.

Which plugin? That is a genuine choice with genuine consequences. **Calico** is a networking and network policy provider supporting overlay and non-overlay networks, with or without BGP. **Cilium** provides a flat Layer 3 network with an eBPF-based data plane, in either native routing or overlay mode, and is a CNCF project at the Graduated level. **Flannel** is an overlay network provider [source: k8s-docs-cluster-addons-2026-08-24]. There are many more.

<!-- AUTHOR-REVIEW: no authoritative source in the cached set states that Kubernetes ships NO network plugin by default, or that a cluster without one has non-functional Pod networking. Outline § Open questions #2 routes this to Stage 2 with a fetch of kubernetes.io/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/. The paragraph below is written to the strength of what IS cached — "the plugin implements pod networking" — and stops short of "and you must install one." Restore the stronger claim once the fetch lands. -->
...
> 🔭 **Closer Look:** The model is a *requirement*, not a description of a mechanism, and that is why implementations differ so wildly. Overlay networks encapsulate Pod traffic inside node-to-node tunnels. Native-routing plugins make Pod addresses routable in the underlying network directly. BGP-based plugins advertise Pod CIDRs to physical routers. eBPF data planes intercept in the kernel. All of them satisfy the same four rules, by completely different means, and an application inside the cluster cannot tell which one it is running on. That last part is the point. Deeper than the exam requires.
...
Rule 2's hedge — *barring intentional network segmentation* — is doing quiet work. Kubernetes has an API for segmenting Pod-to-Pod traffic on purpose, and it is a Chapter 10 subject *[cross-bearing: see Ch 10 §5 — NetworkPolicy]*.
...
**LoadBalancer** exposes the Service externally using an external load balancer [source: k8s-docs-service-2026-08-23] — sitting on top of the NodePort and cluster-IP arrangement beneath it.
...
★ **Fixed Point:** The three ladder types are **additive**. A NodePort Service also has a cluster IP. A LoadBalancer Service sits on top of both. Asking for a higher rung never removes the rungs below it.
...
It is a DNS alias wearing a Service's clothes.
...
<!-- AUTHOR-REVIEW: outline § Open questions #3 flags that `port` vs `targetPort` vs `nodePort`, and the default NodePort range (commonly cited as 30000-32767), are entirely uncached. The outline's recommendation was option (a) — fetch and add a short block — and the fetch has not landed. Per the outline's own guidance ("a half-mention is worse than silence"), this section deliberately says nothing about port fields. If the fetch lands, the natural home is right here, after the decision list. -->
...
A LoadBalancer Service gives you one external address per Service. That is fine for one Service. It is expensive and awkward for fifty, and in most clouds each of those addresses is a billable resource with its own lifecycle.
...
That gap — one address per Service, and no protocol awareness — is exactly what the Gateway API and its predecessor Ingress exist to fill [source: k8s-docs-network-model-2026-08-23]. Chapter 10 opens there. *[cross-bearing: see Ch 10 §1 — the exposure ceiling and what crosses it]*
...
- **"A translated address from the pod network's NAT pool"** assumes cross-node traffic must be translated. Overlay-based plugins do encapsulate the traffic — but encapsulation is not translation, and the Pod at the far end still sees the original source address. That is the plugin's obligation.
...
- **"Yes, they each get their own port space"** is Docker's model, not Kubernetes'. Two containers in one Pod compete for ports exactly as two processes on one host do.
...
Say the rule at the level of the ladder rather than as a fact about NodePort, because the same rule is what makes LoadBalancer comprehensible: **each type adds a layer of reachability on top of the one beneath it.** LoadBalancer implies NodePort implies ClusterIP. Once you hold the rule in that form, you don't have to memorise three separate facts, and Chapter 10's argument for Ingress — "one external address per Service, and that does not scale" — becomes available to you without further work.
...
✓ Four Service types, which three stack, and which one isn't a type of stacking at all
✓ That Kubernetes ships no load balancer

Next: the Service has a name and an address. Now find out what's actually behind it — and what a Pod has to do to get on the list.
...
*[cross-bearing: see Ch 5 §6 — readiness probes]*. That section told you what a readiness probe does. This one tells you where it does it: in the EndpointSlice.
...
So the list is not a boolean membership test. It carries state. A terminating Pod stays on the list, marked, so that anything watching can distinguish "gone" from "going" — which is what lets in-flight connections drain rather than being severed.
...
Two causes. That is the whole diagnostic content of this section, and it is stated as a fact rather than a procedure on purpose — Chapters 13 and 16 own the troubleshooting workflow, and they will retrieve this by name. *[cross-bearing: see Ch 13 §3 — troubleshooting a Service with no endpoints]*
...
One more clause, closing a Chapter 7 loop. That chapter argued that a Service's backends landing on distinct nodes is what makes it resilient rather than merely load-balanced. Now you can see why the argument was necessary: the endpoint list is *just a list*. It has no opinion about where its endpoints are. Topology is the scheduler's problem, which is where you solved it. *[cross-bearing: see Ch 7 §7 — spreading replicas across failure domains]*

<!-- AUTHOR-REVIEW: outline § Open questions #4 — every claim in this section is sourceable, but from five snapshots in combination rather than from EndpointSlice's own documentation (kubernetes.io/docs/concepts/services-networking/endpoint-slices/), which is uncached. Deliberately NOT asserted here: why slices rather than one list, the default endpoints-per-slice limit, and the ready/serving/terminating conditions as a documented set. Do not add any of those without the fetch. -->
...
> 🪝 **Snag:** "Headless" sounds like something went wrong, and a teammate seeing `<none>` in the CLUSTER-IP column will often say so. It means the Service has no *head* — no single virtual IP fronting the set. The Pods are all still there. You just get all of them, instead of one of them.
...
Five questions on what's actually behind the name, and the two times you deliberately don't want one — §4 and §5.
...
*This is the natural end of session one, if you're splitting the chapter.* What remains is the component that actually moves the packet, the names you type, and the observation that ties all of it together.
...
Chapter 3 introduced kube-proxy as a node component — a network proxy that maintains network rules on nodes, implementing part of the Kubernetes Service concept [source: k8s-docs-components-2026-08-23] — and left the "how" for here. *[cross-bearing: see Ch 3 §4 — kube-proxy as a node component]*
...
**Nothing is listening on a cluster IP.** No process is bound to it. There is no host at that address, no socket, no server. It exists only as a rule, replicated on every node, that says: *packets addressed here go to one of those addresses instead.*
...
> ⚓ **Worth Securing:** Nothing is listening on a cluster IP. It is not a host, not a process, not a port on a machine. It is a rule on every node saying *send this somewhere else*. Once that lands properly, roughly half of the confusing behaviour in Kubernetes networking stops being confusing — including why you can't `ping` some cluster IPs usefully, why `netstat` on a node shows you nothing, and why "a Service is a load balancer" was never quite right.
...
| `ipvs` | Linux | |
| `nftables` | Linux | The newest of the three (GA since v1.33) |
...
> 🔭 **Closer Look:** kube-proxy is optional, and increasingly often absent — a plugin like Cilium can do the same work in its own eBPF data plane, usually with better performance characteristics at scale. If you meet a cluster with no kube-proxy Pods in `kube-system`, nothing is missing. Deeper than the exam requires, but useful the first time you see it and assume something is wrong.
...
This section is where you stop needing to know any addresses at all.
...
Chapter 3 promised you CoreDNS by name *[cross-bearing: see Ch 3 §6 — cluster addons and CoreDNS]*, and Chapter 4 gave you a Service's DNS name in a single sentence and explicitly deferred four things: the mechanism, what serves the records, what else gets one, and how resolution actually proceeds *[cross-bearing: see Ch 4 §6 — namespaces and Service DNS names]*. All four are discharged in this section.
...
That is it. That is the entire mechanism. It is not special-case Kubernetes behaviour — it is ordinary DNS search-domain resolution, the same thing that has let you type a short hostname on a corporate network for thirty years. A Pod in `payments` searching for `ledger` tries `ledger.payments.svc.cluster.local` first, because `payments.svc.cluster.local` is in its search list. It finds something. It stops.

★ **Fixed Point:** `<service>.<namespace>.svc.<cluster-domain>`. A **bare name resolves in the client Pod's own namespace only**, because that namespace is in its DNS search list. Crossing a namespace boundary means using the full name.
...
That is the mechanism by which individual StatefulSet members become individually addressable. §5 told you a StatefulSet requires a headless Service for the network identity of its Pods; this is what "network identity" cashes out to — a stable, per-member name that survives rescheduling. *[cross-bearing: see Ch 6 §5 — StatefulSet Pod naming]*
...
kube-proxy configures the node to capture traffic to the Service's `clusterIP` and port, and redirect that traffic to one of the Service's endpoints [source: k8s-docs-virtual-ips-kube-proxy-2026-08-23]. Traffic passes *through* a rule; it is never delivered *to* the cluster IP. There is no socket to find, and `netstat` or `ss` on the node will show you nothing bound there.
...
*A note on what this answer does not say:* it makes no claim about which mode is faster, more scalable, or better. The documentation states the list and the default and says nothing about relative performance, so neither does this book. If you have read that IPVS scales better than iptables for very large Services, that may well be true — but it isn't a claim this material supports, and no exam question you meet should turn on it.
...
**From §1.** And underneath all of it, none of the actual packet-moving is Kubernetes' work at all. A CNI plugin implements the model [source: k8s-docs-extending-kubernetes-2026-08-23] — which is the second instance of an arrangement you first met in Chapter 2, where CRI does the same thing for container runtimes. *[cross-bearing: see Ch 2 §4 — CRI as the first pluggable interface]*
...
That is the chapter's title, arrived at properly. The stable thing was never a Pod — you knew that after §2. But it was never really an IP address either: the cluster IP belongs to nothing, nothing listens on it, and it exists only as a rule that other machinery maintains. What is actually stable is *"which Pods carry the label `app: db` and are Ready"* — a question whose answer is different every few minutes and whose **meaning** is the same forever. Pods churn underneath it. The question doesn't move.
...
4. **NodePort also allocates a cluster IP** — the types are additive [source: k8s-docs-service-2026-08-23].
...
12. **Kubernetes defines the network model and implements none of it** — a CNI plugin does [source: k8s-docs-extending-kubernetes-2026-08-23].

---

**Common Traps** — each of these is a documented mistake, and each has a specific correct answer. Don't let the wrong version be the one that's fresh when you sit down.
...
| "Kubernetes ships the network" | It ships the *requirements*; a CNI plugin ships the network | §1 Fixed Point |
...
Twenty-one questions. Four of them draw on earlier chapters and are tagged. Answers and explanations follow the full set — attempt them all before looking.
...
B) CNI is the interface through which a network plugin implements Pod networking; the plugin is a binary the kubelet executes
...
**4.** Why does a Pod's IP address make a poor thing for a client to store in its configuration?

A) Pod IPs are randomly rotated by the kubelet on a fixed interval
B) Pod IPs are only routable from the node the Pod runs on
C) A Pod is never repaired in place — when it is replaced, the replacement is a different Pod with a different address
D) Pod IPs are behind NAT and are not the address other Pods actually see

---

**5.** A Service is created with no `type` field specified. What type is it, and from where is it reachable?
...
**6.** You change a Service from `type: ClusterIP` to `type: NodePort`. What happens to the cluster IP?
...
**7.** Your team runs Kubernetes on bare metal with no cloud-provider integration and no load-balancer add-on installed. You create a `type: LoadBalancer` Service. What happens?
...
**8.** Which Service type sets up **no proxying of any kind**?
...
**9.** You need clients inside the cluster to reach `analytics.vendor.example`, a SaaS endpoint outside the cluster, using an in-cluster name. Which Service configuration produces a CNAME with Kubernetes entirely out of the traffic path?
...
**10.** **[retrieval: ch4]** A Pod carries labels that a ReplicaSet selects on and labels that a Service selects on. Someone edits one of those labels. Name both consequences, and say whether the two controllers coordinate.
...
C) The Pod may leave the ReplicaSet's set and the Service's endpoint list independently; the controllers do not coordinate, because they are independent queries over the same labels
D) Nothing happens until the Pod is restarted, at which point both selectors re-evaluate

---

**11.** Where does Kubernetes record the set of Pods currently backing a Service, and what writes it?
...
**12.** **[retrieval: ch5]** Chapter 5 said a readiness probe controls whether a Pod receives traffic. Name the object that fact is actually implemented in, and say what a failing probe changes about it.
...
**13.** A Service's selector matches six Pods. Four are Ready, one is failing its readiness probe, and one is terminating gracefully. What does the Service's EndpointSlice contain?

A) Four entries; the failing and terminating Pods are both absent
B) Six entries; readiness affects only kube-proxy, not the slice
C) Five entries — four ready, plus the terminating Pod retained with `ready: false`
D) Four entries plus the failing Pod marked `ready: false`

---

**14.** `kubectl get endpointslices -l kubernetes.io/service-name=web` returns a slice with no endpoints. The Pods you expect are Running. What are the two usual causes?
...
C) The Pods are on nodes without a cluster IP allocated, or DNS has not propagated
D) The Service has no `type` set, or the EndpointSlice controller is disabled

---

**15.** A colleague reports: "I created the Service, it has a cluster IP and a DNS record, but nothing responds and there are zero endpoints." What is the correct characterisation?
...
**16.** What does setting `.spec.clusterIP` to `None` do, and which workload type requires it?

A) Deletes the Service's address after creation; required by DaemonSets
B) Makes the Service internal-only; required by Deployments with more than one replica
...
D) Disables kube-proxy for that Service only; required by Jobs

---

**17.** You are migrating a database into the cluster. Today it runs on external hardware; next month it will run as Pods. You want in-cluster clients to use one unchanging Service name throughout, with traffic proxied by the cluster in both phases. What do you configure today?
...
D) A `type: LoadBalancer` Service with the external address in an annotation

---

**18.** kube-proxy's modes on Linux are:
...
**19.** An engineer SSHes to a worker node and runs `ss -tlnp` looking for a process bound to a Service's cluster IP. What will they find, and why?
...
**20.** Your cluster is running normally, serving traffic to Services, and there are no kube-proxy Pods anywhere. What is happening, and which of §1's four rules is the same component also satisfying?
...
Network plugins use CNI, the Container Network Interface, to implement pod networking, and CNI network plugins are binary plugins executed by the kubelet [source: k8s-docs-extending-kubernetes-2026-08-23].
...
A Pod is never rescheduled to a different node; it is replaced by a new, near-identical Pod with a different UID [source: k8s-docs-pod-lifecycle-2026-08-23], and each Pod gets its own address [source: k8s-docs-network-model-2026-08-23]. The set of Pods running at one moment can be different from the set running a moment later [source: k8s-docs-service-2026-08-23].

**A** invents a rotation mechanism that doesn't exist — addresses are stable *for the life of the Pod*. **B** contradicts rule 2. **D** contradicts rule 3.
...
**C** is the near-miss worth naming: ClusterIP alone does *not* expose anything on node IPs. That is what NodePort adds — and the additivity runs one way only. Getting a cluster IP from NodePort is guaranteed; getting node reachability from ClusterIP is not.
...
Note that the Service still has a cluster IP and, per the additive rule, a node port. It is fully usable from inside the cluster and from the nodes. Only the external address is missing.

**8. D.**

ExternalName maps the Service to the contents of the `externalName` field by configuring the cluster's DNS to return a CNAME record with that external hostname value; **no proxying of any kind is set up** [source: k8s-docs-service-2026-08-23].

**A**, **B** and **C** all involve kube-proxy programming interception rules [source: k8s-docs-virtual-ips-kube-proxy-2026-08-23]. That is exactly the set kube-proxy covers: every type *other than* ExternalName.
...
**A** puts the answer in the wrong object and names the wrong writer — kube-proxy *reads* EndpointSlices [source: k8s-docs-virtual-ips-kube-proxy-2026-08-23]. **D** confuses a publisher with a source of truth: DNS publishes the answer; it does not compute it.
...
The four Ready Pods are endpoints. The failing Pod is excluded [source: k8s-docs-pod-lifecycle-2026-08-23]. The terminating Pod is the interesting one: endpoints representing terminating Pods are **not immediately removed** from EndpointSlices, terminating state is exposed via the EndpointSlice API, and terminating endpoints always have `ready: false` so load balancers will not use them for regular traffic [source: k8s-docs-pod-termination-2026-08-24].

**A** treats terminating as immediate removal — the plausible-sounding answer, and the reason graceful draining would not work if it were true. **B** ignores readiness entirely. **D** inverts which Pod gets retained-and-marked.

**14. B.**

If the endpoints don't match the Pods you expect, the Service's selector probably does not match the Pods' labels, or the Pods are not Ready [source: k8s-docs-debug-pods-2026-08-23].

**A** names data-plane components, but endpoint membership is computed entirely in the control plane by a controller reading the API. Neither kube-proxy nor the CNI plugin can empty a slice. **C** and **D** invent failure modes.
...
**A**, **B** and **D** each invent a behaviour and attach it to the wrong workload type. Note in particular that headless has nothing to do with internal-versus-external reachability — that axis is the type ladder, and it is independent of this one.
...
**A** would give clients the vendor hostname directly, meaning that when you migrate you must change what clients resolve *and* the connection semantics change — no proxying today, proxying tomorrow. The question specifically asks for proxying in both phases. **B** removes the single address, which the clients want. **D** is invented.

The migration itself, when it comes, is: add the selector, delete the manual EndpointSlices. Clients notice nothing.
...
**A** names a retired mode. **C** mislabels the default and lists eBPF, which is a plugin data plane rather than a kube-proxy mode. **D** mislabels the default and places the Windows-only mode in the Linux list.
...
| **CNI** | Kubernetes **defines** the model; a **plugin implements** it. Second of the four pluggable interfaces |
...
| **LoadBalancer** | On top of NodePort. **Kubernetes supplies no load balancer** |
...
It also opens with a Service that you create, and that does nothing at all — because the component that acts on it has not been installed. You have seen that shape twice in this chapter now. Chapter 10 gives it a name, and once it has a name you will start seeing it everywhere.

> *"You do not need to know where a thing is. You need to know what to call it, and who keeps the answer current."*
> — Lodestar Ledgers