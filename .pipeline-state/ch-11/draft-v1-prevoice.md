...
**Chapter allocation within domain: ~5% of total exam (authored allocation — CNCF publishes four domain weights and no sub-competency weights) | Complexity: Mixed | Novelty: Moderate**

---

## Attention Budget

**Total time: ~85 minutes | Recommended: Split across 2 sessions, with the break after §4**
...
| §4 Access Modes and What Happens After | 14 min | Medium | Peak attention — highest exam yield |
...
*If you only have 15 minutes: read §2 and §4, then take ☆ Taking Your Bearings #2. That is where the exam points live.*
...
Before reading this chapter, try these eight questions. Your score determines how to approach the content — there is no shame in any score, only different reading strategies.
...
5. **The storage administrator creates the LUN; the application team requests one.** The requester specifies a size and a performance need. They do not have to know whether the array is NetApp, Pure, or a pile of disks in a closet — and that separation is the entire point of the arrangement.
...
7. **It depends — and the "depends" is the interesting part.** A boot disk created automatically with the VM usually dies with it. A disk you provisioned separately and attached usually survives. The provisioning history determines the deletion behavior.

8. **No, not safely.** Two independent filesystem drivers each believing they own the block device will corrupt it — each caches metadata the other doesn't know it changed. A clustered filesystem exists precisely to coordinate that.

**If you got 6+ right:** Skim this chapter. Focus on the ★ Fixed Points and the ⚠ Navigational Hazards callouts, and go straight to ☆ Taking Your Bearings #2 — that is where the exam yield concentrates and where a confident reader is still most likely to lose points.
...
>
> The hold is inventoried, claimed, and released by a process that runs on entirely different rails from the watch rotation. A shipper files a claim against space in the hold. A quartermaster decides which part of the hold satisfies it. When the claim is released, what happens next — is the cargo landed, is it held for the next voyage, is it destroyed — was settled by the terms of the arrangement long before anyone came to collect it.
>
...
- **Distinguish** a PersistentVolume from a PersistentVolumeClaim from a StorageClass — the three-way distinction the exam blueprint names explicitly — and say which one a Pod actually references.
...
Rung three is what is left when you ask for storage whose lifetime is not tied to any Pod at all. **Kubernetes does not destroy persistent volumes** [source: k8s-docs-volumes-2026-08-23]. That is the entire distinction, and §2 onward is about how you get one.
...
**`hostPath`** mounts a file or directory from the host node's filesystem into your Pod [source: k8s-docs-volume-types-depth-2026-08-25]. It has an optional `type` field alongside the required `path`, which controls what checks Kubernetes performs before mounting — whether the path must already exist, must be a directory, may be created, must be a socket, and so on [source: k8s-docs-volume-types-depth-2026-08-25]. The documentation's own framing of `hostPath` is worth quoting because it is exactly right: *this is not something that most Pods will need, but it offers a powerful escape hatch for some applications* [source: k8s-docs-volume-types-depth-2026-08-25].
...
>
> Even a read-only restriction is conditional: *if you are restricting access to specific directories on the node using admission-time validation, that restriction is only effective when you additionally require that any mounts of that `hostPath` volume are read only* [source: k8s-docs-volume-types-depth-2026-08-25]. Allow a read-write mount to an untrusted Pod, and *the containers in that Pod may be able to subvert the read-write host mount* [source: k8s-docs-volume-types-depth-2026-08-25].
>
...
>
> This is the workload-to-host boundary problem, and it is why an entire security apparatus exists to police it. We are naming it here and leaving it: *[cross-bearing: see Ch 12 §5 — what a Pod may do to its node]*.
...
**`projected`** is the multiplexer. *A `projected` volume maps several existing volume sources into the same directory* [source: k8s-docs-projected-volumes-2026-08-25]. The sources that can be projected are `secret`, `downwardAPI`, `configMap`, `serviceAccountToken`, `clusterTrustBundle`, and `podCertificate` [source: k8s-docs-projected-volumes-2026-08-25].
...
The token case has its own parameters worth knowing: the `audience` field carries the intended audience of the token, and `expirationSeconds` sets its validity duration — *it defaults to 1 hour and must be at least 10 minutes (600 seconds)* [source: k8s-docs-projected-volumes-2026-08-25].
...
Here is the mechanism, and it is worth reading twice because it prefigures §6. The Pod spec carries a full PersistentVolumeClaim template inline. When such a Pod is created, *the ephemeral volume controller then creates an actual PersistentVolumeClaim object in the same namespace as the Pod and ensures that the PersistentVolumeClaim gets deleted when the Pod gets deleted* [source: k8s-docs-ephemeral-volumes-2026-08-25]. A real claim, created for you, garbage-collected with the Pod. Rung two behavior, built out of rung three machinery.
...
Now the exception. Chapter 4 hedged that a mounted ConfigMap picks up changes when the ConfigMap is updated, and told you the exception had a name and would arrive here *[cross-bearing: see Ch 4 §4 — configuration kept outside the image]*. Here it is, and it is a flat rule with no conditions attached: *a container using a ConfigMap as a `subPath` volume mount will not receive updates when the ConfigMap changes* [source: k8s-docs-volume-types-depth-2026-08-25].
...
It is not. The documentation describes a PersistentVolume as being *volume plugins like Volumes, but* with *a lifecycle independent of any individual Pod that uses the PV* [source: k8s-docs-persistent-volumes-2026-08-23] — and that "like Volumes" is precisely what invites the confusion. A `volume` is a field in a PodSpec. A `PersistentVolume` is a cluster-scoped API object with its own name, its own lifecycle, and its own existence independent of every Pod in the cluster. Different things. Similar words. Keep them apart.

With that out of the way, here is the arrangement, and the cleanest way in is the documentation's own proportion:
...
Which brings us to the routing detail that this whole section exists to fix.
...
D) To a Pod-scoped volume created implicitly by the kubelet; still there
...
B) `Released` — the claim is gone, but the resource is not yet reclaimed by the cluster
...
**1 — C.** Present after the crash; present after the delete… no. Read that again — the correct answer is **B**.

Let me be direct, because this is the trap and I just walked into it deliberately so you would see the shape of it. The answer is **B: present after the crash, gone after the delete.**
...
- **B is correct.** Crash → survives (rung two beats a container restart). Delete → gone, because *when a Pod is removed from a node for any reason, the data in the `emptyDir` is deleted permanently* [source: k8s-docs-volume-types-depth-2026-08-25]. And note that the replacement Pod is a *different Pod* — its `emptyDir` is created empty when it is assigned to a node.
...
- **D is wrong** in both halves — it has the ladder exactly inverted.
...
- **D is wrong** — the kubelet does not create implicit volumes. A volume exists because a PodSpec declared it.
...
- **C is wrong** — `Failed` means *the volume has failed its automatic reclamation* [source: k8s-docs-persistent-volumes-depth-2026-08-25], which is a different event.
...
✓ The four PV phases, and that `Released` is not `Available`
...
🗺️→🌊→🌅 · **Voyage Progress: you are through the harbor and into open water.**
...
The problem it solves is stated back in the PersistentVolumes documentation, and it is worth reading as a problem statement rather than a feature description: *cluster administrators need to be able to offer a variety of PersistentVolumes that differ in more ways than size and access modes, without exposing users to the details of how those volumes are implemented. For these needs, there is the StorageClass resource* [source: k8s-docs-persistent-volumes-2026-08-23].
...
Most clusters do not make every developer name a class explicitly, because most clusters have a default. *You can mark a StorageClass as the default for your cluster. When a PVC does not specify a `storageClassName`, the default StorageClass is used* [source: k8s-docs-storage-classes-2026-08-25]. The mark is an annotation: `storageclass.kubernetes.io/is-default-class: "true"` [source: k8s-docs-persistent-volumes-depth-2026-08-25].

That is the common case, and it is why a developer can write a five-line PVC on a managed cloud cluster and get a working disk without knowing any of this.
...
A word before we start, because the difficulty glyph on this section and its exam yield are pulling in opposite directions, and you should calibrate on the yield.
...
>
> There are three reclaim policies, one of which is deprecated. `Retain`: when the claim is deleted the PV still exists, the volume is considered released, and an administrator must manually reclaim it. `Delete`: removes both the PersistentVolume object from Kubernetes and the associated storage asset in the external infrastructure. `Recycle`: deprecated [source: k8s-docs-persistent-volumes-2026-08-23]. Valid options and their defaults: Retain is the default for manually created PersistentVolumes; Delete is the default for dynamically provisioned PersistentVolumes; Recycle is deprecated [source: k8s-api-ref-persistentvolume-v1-2026-08-25].

Now the parts that need explaining.

### Access modes count nodes

The Soundings asked you whether two machines can safely mount one block device read-write at the same time. They cannot — not without a clustered filesystem coordinating them, because two independent filesystem drivers each caching metadata for the same device will corrupt it.
...
>
> The through-line: every one of these is a wrong guess about a default someone else set. That is what makes them dangerous rather than merely wrong — you cannot catch them by reading your own manifest.
...
⚠️ **These questions are intentionally challenging.** Struggle is the point here — four of this chapter's seven exam traps are concentrated in the material you just read, and that concentration is not accidental. It is where the material is genuinely counter-intuitive.
...
C) PVC `Bound`, Pod `Running` on a node that exceeds its allocatable capacity
D) PVC `Failed`, Pod `Failed` — an unschedulable Pod causes its claim to fail reclamation
...
- **D is wrong** — `Retain` does not *attempt* automatic reclamation, so it cannot fail at it.
...
One question left, and it is the one that has been implicit since §3: when a provisioner creates a volume, what is actually doing the creating?
...
The reason CSI was necessary is worth one paragraph, because it makes the argument concrete rather than architectural.
...
Sit with the consequences of that. A storage vendor wanting Kubernetes support had to submit code to the Kubernetes project, have it reviewed by Kubernetes maintainers, and wait for a Kubernetes release. Their bug fixes shipped on Kubernetes' schedule. Their code ran inside Kubernetes' binaries, and a defect in it was a defect in the kubelet. Meanwhile, Kubernetes maintainers were reviewing and carrying storage code for hardware they did not own and could not test.
...
That shape should be recognizable without further explanation. One controller, running somewhere, handling the cluster-wide operations — create this volume, delete that one *[cross-bearing: see Ch 6 §1 — the resource that holds the intent]*. One agent per node, running everywhere, handling the node-local operations — mount this volume into this path on this machine, and DaemonSet is exactly the workload type for "one per node" *[cross-bearing: see Ch 6 §7 — one per node, and work that ends]*. A CSI driver is not exotic infrastructure. It is a Deployment and a DaemonSet, running software somebody else wrote.
...
Once installed, the driver's volumes are usable in three ways: *through a reference to a PersistentVolumeClaim*, *with a generic ephemeral volume*, or *with a CSI ephemeral volume if the driver supports that* [source: k8s-docs-volumes-csi-and-subpath-2026-08-25]. The first is the path §2 through §4 described.
...
>
> KCNA asks you to name the storage interface and say what it is for. The following is legitimately deeper than the exam requires, and is here for the day you meet it in a cluster rather than on a test.
>
...
**For each VolumeClaimTemplate entry defined in a StatefulSet, each Pod receives one PersistentVolumeClaim** [source: k8s-docs-statefulset-2026-08-24].
...
*The same PersistentVolumeClaim will be bound to a Pod throughout its lifecycle* [source: k8s-docs-statefulset-2026-08-24]. And then: *when a Pod is (re)scheduled onto a (different) Node, its volumeMounts mount the PersistentVolumeClaim(s) associated with its PersistentVolume(s)* [source: k8s-docs-statefulset-2026-08-24].

Note what that sentence does *not* say. It does not say the storage moves. It does not say the data is copied. It says the mount happens wherever the Pod lands — because the Pod's storage is attached to *the claim*, and the claim is not attached to a node at all. The claim is a namespaced API object with no node in it. The volume behind it is cluster-scoped. Neither one cares which machine `web-1` is running on today.

That is why a StatefulSet can survive a node failure with its data intact, and it is worth stating explicitly because it explains something otherwise mysterious: the identity you learned in Chapter 6 is not just a naming convention. `web-1` is a name that carries a claim with it. Reschedule the Pod and the claim comes along, because the claim was never anywhere else.
...
>
> The documentation states it twice. In the Stable Storage section: *the PersistentVolumes associated with the Pods' PersistentVolume Claims are not deleted when the Pods, or StatefulSet are deleted. This must be done manually* [source: k8s-docs-statefulset-storage-2026-08-25]. And as a policy default: *Retain (default): PVCs from the volumeClaimTemplate are not affected when their Pod is deleted*, with *the default for policies is Retain* [source: k8s-docs-statefulset-storage-2026-08-25].
>
...
Read that as a judgment call, because that is what it is. Kubernetes chose the failure mode where you pay for storage you forgot about over the failure mode where a `kubectl delete` destroys a database. Given the two, it chose correctly — but it means the cleanup is yours, and nobody will remind you.
...
>
> One boundary worth knowing: these policies apply only to *voluntary* removal. *If a Pod associated with a StatefulSet fails due to node failure, and the control plane creates a replacement Pod, the StatefulSet retains the existing PVC. The existing volume is unaffected, and the cluster will attach it to the node where the new Pod is about to launch* [source: k8s-docs-statefulset-storage-2026-08-25]. A node dying is not a scale-down. Your data does not get cleaned up because a machine crashed.
>
...
Five questions on CSI, the interface pattern, and the StatefulSet pairing.
...
**2.** An administrator creates a StorageClass whose `provisioner` field names `ebs.csi.aws.com`. The corresponding CSI driver has never been deployed to the cluster. A developer creates a PVC requesting that class and a Pod that mounts it. What is observed?

A) The API server rejects the StorageClass at creation time, since the provisioner does not exist
B) The PVC binds to a placeholder PV and the Pod starts with an empty volume
...
**5.** Which of the following is *not* one of the four pluggable interfaces this book has now collected, and which chapter introduced the one you would substitute for it?

A) CRI — Chapter 2
B) CNI — Chapter 9
C) CSI — Chapter 11
D) CIS — none; the fourth is CRDs, introduced in Chapter 6
...
- **B is wrong** — there is no placeholder PV mechanism. An unmatched claim stays unbound indefinitely [source: k8s-docs-persistent-volumes-2026-08-23].
- **D is wrong**, and stating why is worth more than the right answer: nothing in Kubernetes installs drivers. This is the fourth instance of the pattern named at Ch 10 §3.

**3 — B.** *The PersistentVolumeClaim(s) associated with the Pod's PersistentVolume(s) are not deleted when the Pod, or the StatefulSet is deleted. This must be done manually* [source: k8s-docs-statefulset-2026-08-24]. The naming — `<template>-<statefulset>-<ordinal>` — tells you the set was called `web` and had a template called `www`. What it says about identity and storage: the claim is bound to the *name*, not to the workload object, which is why deleting the workload does not disturb it.
...
**5 — D.** The four are CRI (Chapter 2), CRDs (Chapter 6), CNI (Chapter 9), and CSI (Chapter 11) [source: k8s-docs-extending-kubernetes-2026-08-23]. "CIS" is not one of them.
- **A, B, and C** are all genuine members and are correctly attributed.
- The substitution is **CRDs**, introduced at Chapter 6 §8 as the extension point at the API itself. Chapter 17 §4 collects all four and adds the wider set of extension points the Kubernetes documentation enumerates *[cross-bearing: see Ch 17 §4 — every place Kubernetes lets you in]*.

---

**If you scored 0–2:** Re-read §5 and §6 — about eighteen minutes. If question 5 was the miss, the gap is not this chapter at all; it is the running count of interfaces, and Chapter 10's closing pages restate it compactly.

**If you scored 3–4:** Good. You have the interface pattern and the StatefulSet pairing.

**If you scored 5:** You have closed the book's one deliberate forward reference and you hold all four interfaces. Chapter 17 will be a collection exercise rather than a learning one.
...
✓ `volumeClaimTemplates` and the one-claim-per-Pod rule
...
1. **PersistentVolume vs PersistentVolumeClaim vs StorageClass.** The KCNA domain expectation names this three-way distinction explicitly, which makes it the highest-confidence claim in this chapter's exam surface. Know: PV is supply and cluster-scoped; PVC is demand and namespaced; StorageClass describes *classes* of storage and enables dynamic provisioning. And know that a Pod references the claim [source: k8s-docs-persistent-volumes-2026-08-23].
...
**Common Traps** — these are documented exam targets, not merely things that are easy to confuse:
...
- **A StatefulSet's PVCs survive deletion of the Pod and of the StatefulSet**, and must be deleted manually [source: k8s-docs-statefulset-2026-08-24].
...
**1.** A Pod mounts an `emptyDir` volume at `/cache` and a `secret` volume at `/etc/creds`. The node the Pod is running on is drained and the Pod is deleted and recreated elsewhere. What is the state of the contents of each path on the new Pod?

A) `/cache` empty; `/etc/creds` populated from the Secret
B) `/cache` preserved; `/etc/creds` populated from the Secret
C) `/cache` empty; `/etc/creds` empty until the Secret is remounted manually
D) Both preserved, since both are Pod-scoped volumes
...
D) The cluster must have `Recycle` enabled as the default reclaim policy
...
D) No claims until a Pod writes data, at which point one is created per write
...
**13.** A PVC is created with `storageClassName: ""` on a cluster that has a default StorageClass and no classless PersistentVolumes. What is the outcome?

A) It uses the default StorageClass
B) It is rejected as an invalid manifest
C) It stays unbound, because it has opted out of dynamic provisioning and no classless PV exists
D) It binds to any `Available` PV, since an empty class matches all
...
**1 — A.** `/cache` is empty: *when a Pod is removed from a node for any reason, the data in the `emptyDir` is deleted permanently* [source: k8s-docs-volume-types-depth-2026-08-25], and the new Pod's `emptyDir` is created empty when it is assigned to a node. `/etc/creds` is populated because a `secret` volume's content comes from the Secret object, which is cluster state independent of any Pod.
- **B** confuses rung two with rung three. **C** misunderstands where a Secret's data lives — the object persists; the mount is rebuilt from it. **D** is the rung-two-is-persistent misconception applied to both paths at once.
...
- **A**, **C**, and **D** each get at least one half wrong. **D** inverts both.
...
- **B** describes static provisioning — in fact dynamic provisioning is what happens when *no* matching PV exists. **C** invents a field usage; `volumeName` binds to an existing PV. **D** is unrelated and names a deprecated policy.
...
- **B**, **C**, and **D** are all invented patterns. **C** in particular is worth rejecting explicitly: the API server validates schema, not the existence of running components.
...
**10 — C.** *For each VolumeClaimTemplate entry defined in a StatefulSet, each Pod receives one PersistentVolumeClaim* [source: k8s-docs-statefulset-2026-08-24] — hence two, named `data-db-0` and `data-db-1`. And they are *not deleted when the Pod, or the StatefulSet is deleted. This must be done manually* [source: k8s-docs-statefulset-2026-08-24].
- **A** describes a shared claim, which is what a Deployment referencing a PVC would give you. **B** gets the naming right and the survival wrong — the more expensive half. **D** is fabricated.
...
**12 — C.** Each of the four publishes a contract so an implementation can be supplied from outside. CSI states it directly — vendors can *introduce new storage systems into Kubernetes without ever having to edit the core Kubernetes code* [source: k8s-docs-volumes-2026-08-23] — and the extension-points documentation groups CSI, CNI, and CRI together as infrastructure extensions alongside CRDs as API extensions [source: k8s-docs-extending-kubernetes-2026-08-23].
...
**13 — C.** *Claims that request the class `""` effectively disable dynamic provisioning for themselves* [source: k8s-docs-persistent-volumes-2026-08-23], and such a claim *is explicitly stating that it is bound with a PV with no class, hence the PV's `storageClassName` must also be empty* [source: k8s-docs-persistent-volumes-depth-2026-08-25]. No such PV exists.
- **A** is the trap: the default applies when the field is *absent*, not when it is empty. **B** is false — the value is valid. **D** inverts the meaning entirely.
...
**16 — B.** *When a Pod is (re)scheduled onto a (different) Node, its volumeMounts mount the PersistentVolumeClaim(s) associated with its PersistentVolume(s)* [source: k8s-docs-statefulset-2026-08-24]. Identity is stable across rescheduling *[cross-bearing: see Ch 6 §6 — when Pods are not interchangeable]*, so the name is preserved and the claim comes with it.
...
| **Binding** | A control loop match. Exclusive, one-to-one. An unmatched claim stays unbound indefinitely — not an error, just silence. |
| **PV phases** | `Available` → `Bound` → `Released` → (`Failed`). `Released` is not `Available`. |
