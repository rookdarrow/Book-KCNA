I can't write files in this session — the harness is holding this stage to stdout, as the pipeline intends. Emitting the document.
...
5. **The storage administrator creates the LUN; the application team requests one.** The requester specifies a size and a performance need. They do not have to know whether the array is NetApp, Pure, or a pile of disks in a closet. That separation is the entire point of the arrangement.
...
**`hostPath`** mounts a file or directory from the host node's filesystem into your Pod [source: k8s-docs-volume-types-depth-2026-08-25]. It has an optional `type` field alongside the required `path`, which controls what checks Kubernetes performs before mounting: whether the path must already exist, must be a directory, may be created, must be a socket, and so on [source: k8s-docs-volume-types-depth-2026-08-25]. The documentation's own framing of `hostPath` is exactly right: *this is not something that most Pods will need, but it offers a powerful escape hatch for some applications* [source: k8s-docs-volume-types-depth-2026-08-25].
...
The token case has its own parameters. The `audience` field carries the intended audience of the token, and `expirationSeconds` sets its validity duration: *it defaults to 1 hour and must be at least 10 minutes (600 seconds)* [source: k8s-docs-projected-volumes-2026-08-25].
...
- **B** confuses rung two with rung three. **C** misunderstands where a Secret's data lives; the object persists, and the mount is rebuilt from it. **D** is the rung-two-is-persistent misconception applied to both paths at once.
...
- **A** describes a shared claim, which is what a Deployment referencing a PVC would give you. **B** gets the naming right and the survival wrong, which is the more expensive half. **D** is fabricated.
