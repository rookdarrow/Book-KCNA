...
Almost everyone reaches for the same thing: `kubectl logs`. It is the obvious move. The application is misbehaving, applications write logs, so read the logs. Sometimes that instinct is right. But it is wrong in the three most common failure shapes there are, and in each of them it costs you the first ten minutes of the investigation and, worse, hands you a confident wrong answer.
