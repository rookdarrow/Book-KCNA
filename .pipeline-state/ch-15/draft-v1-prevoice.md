...
This chapter leans harder on what came before than any other in the book. Two of these questions in particular are load-bearing: if you miss them, the last section will not land, and no amount of careful reading here will fix that. Better to find out now.
...
You have a unit now — a chart, a set of overlays, something with a version number that installs the same way twice. And it has bought you less than it should have, because the unit still gets applied by a person. Somebody with cluster credentials on their laptop, running a command, from a machine nobody audits, at a moment nobody records. They apply the version they believe is current. Afterward, nothing keeps watch.

That complaint has two halves, and this chapter answers them in order.

The first half is **who applies it**. That sounds like a procedural question — a matter of team convention, the sort of thing settled in a runbook. It isn't. The answer determines where cluster credentials physically live, who can reach them, and how large a single mistake gets before something catches it. Change the answer and you change the security posture of the whole delivery path. We take that up in §3.
...
So the reader already owns the answer to the second half. They have not yet been shown that they own it. That is what §7 is for, and it is why this chapter's subtitle gives the answer away on the second line — because the recognition is worth more than the surprise.
...
And when the constraint genuinely does not hold — when the replicas are *not* interchangeable — Kubernetes has a different object for you, and it is a different object precisely because it is stepping outside this factor *[cross-bearing: see Ch 6 §6 — when Pods are not interchangeable]*.
...
You may have heard **A/B testing** listed alongside these four. This book does not include it, deliberately, and the reason is worth a sentence because it is a real distinction rather than an omission.
...
Note the asymmetry, because it is the interesting part: cluster-wide **read** is structural — the agent cannot detect drift in something it cannot see — while broad **write** is a default, not a requirement.
...
**Q11. D.** Principle 4 requires that agents *"continuously observe actual system state and attempt to apply the desired state"* [source: opengitops-principles-v1-2026-08-31]. A system that acts only on new commits satisfies principle 3 — it does pull — but has no answer to drift introduced any other way. A manual `kubectl edit` would persist indefinitely, which is exactly the failure GitOps exists to close.
