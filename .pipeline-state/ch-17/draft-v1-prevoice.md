...
*If you only have 15 minutes: read §4 and §9 together, then take Taking Your Bearings 2.*

*The natural split point is after Taking Your Bearings 2. §1 through §5 are the ecosystem's vocabulary; §6 through §9 are its shape.*
...
**4.** A sidecar is a second container in the same Pod, running alongside the main application container. It shares the Pod's network namespace — meaning it reaches the main container on `localhost` — and can share the Pod's volumes. *[cross-bearing: see Ch 5 §2 — more than one container aboard]*.

**5.** The HorizontalPodAutoscaler object is created successfully. Nothing scales. The HPA needs the Metrics API to read utilization from, and on a cluster with no metrics-server there is nothing serving that API. The object exists; the component that would act on it does not. *[cross-bearing: see Ch 13 §7 — numbers nobody collects by default]*.
...
**7.** The primary durable unit is a **SIG** — Special Interest Group. A group formed to work across several of them is a **Working Group**. If you got the first and guessed at the second, you scored exactly what most readers score here, and §8 is where the guess becomes knowledge.
...
Four hundred pages ago, Chapter 1 used a phrase and refused to define it. Here is the document it was refusing to paraphrase.
...
**The characteristics clause.** This is the part the exam can quote at you, so it is the part worth knowing verbatim.
...
> 🪝 **Snag:** Do not treat those seven as a closed set. The definition says outright that they are not one. It is a natural reader error — a list of exactly seven things, printed in an authoritative document, looks like a complete enumeration — and it is a natural exam trap for exactly the same reason. What makes it worth flagging here specifically is that §4 of this chapter *does* teach a set that genuinely is closed. Two lists, three sections apart, and only one of them is finite.
...
The Cloud Native Computing Foundation is part of the nonprofit Linux Foundation, and its mission is stated in one line: **to make cloud native computing ubiquitous** [source: cncf-charter-governance-bodies-2026-08-31]. It hosts critical components of global technology infrastructure — Kubernetes, Prometheus, Envoy among them — 227 projects at the time of this book's research, across four categories, with 715 member organizations and over 329,000 project contributors [source: cncf-who-we-are-2026-08-23].
...
There is a fourth word you will meet if you browse cncf.io, and it is not a rung. The TOC's own process documentation lists **Archived** — "inactive or low activity projects that are no longer supported" — alongside the other three [source: cncf-toc-project-lifecycle-process-2026-08-31], and the foundation counts its projects across "the Graduated, Incubating, Sandbox, and Archived categories" [source: cncf-who-we-are-2026-08-23]. Archived is a terminal state, not a step up. Projects do not climb to it; they land there when nobody is maintaining them any more. The three-rung progression is correct as a progression, and Archived is the exit.

> ⚓ **Worth Securing:** Learn the *levels*, not the roster. Which specific projects are Graduated on the day you sit the exam is a moving target — the foundation graduates several projects a year — and no responsible study guide should ask you to memorize a list that changes faster than it prints. What does not move is what each level *asserts*, and that is what a well-built question tests.

That said, a ladder with nothing standing on it is hard to remember. Here are six Graduated projects you have already met by name in this book, as of the 2026-08-23 snapshot of the CNCF projects page [source: cncf-project-maturity-levels-2026-08-23]: **containerd**, **CoreDNS**, **etcd**, **Helm**, **Prometheus**, and **Argo**. Kubernetes itself is Graduated too, which is worth a moment's thought — it climbed the same ladder as everything else. Do not memorize even this short list. It is here so the word *Graduated* has weight, not so you can recite it.

### Where the criteria actually live

> 🪝 **Snag:** The projects page tells you what the levels *mean*. It does not tell you what a project must *do* to move between them. The criteria — the due diligence, the adopter interviews, the governance and security requirements — live in the **CNCF TOC's project lifecycle documentation**, in the TOC's own repository [source: cncf-project-maturity-levels-2026-08-23]. A question that asks where graduation criteria are defined is testing whether you know these are two different documents.

The process itself is more concrete than most people expect. A project applies by opening an application issue on the TOC repository, and completes an adopter interview form with **five to seven adopters willing to be interviewed** [source: cncf-toc-project-lifecycle-process-2026-08-31]. A TOC sponsor is assigned. There is a kickoff meeting, a due diligence document, actual interviews with those adopters, a one-week internal TOC comment period, and then a **two-week public comment period**. Finally the TOC votes, and it takes a **two-thirds supermajority** to pass [source: cncf-toc-project-lifecycle-process-2026-08-31].
...
> ⚓ **Worth Securing:** The CNCF's own glossary says a well-designed monolith "can uphold lean principles by being the simplest way to get an application up and running." That is the foundation for cloud native computing saying, in writing, that microservices are not always the right answer. Take it as a licence to think rather than a licence to argue — the exam tests what microservices *are* and what they trade away, not which you should pick.
...
You have known this one since Chapter 4 and it does not need re-teaching. What it needs is placement: **declarative APIs are named in the CNCF definition as a cloud native technology**, alongside containers and microservices. The thing you have been doing since Chapter 4 — describing desired state and letting a controller reconcile — is not a Kubernetes quirk. It is a named characteristic of the whole paradigm. *[cross-bearing: see Ch 4 §1 — you file a declaration]*.
...
A) Cloud native workloads run in public cloud environments
B) Cloud native systems are characterized by loosely coupled systems that interoperate securely, resiliently, manageably, sustainably, and observably
C) Cloud native architecture requires a service mesh
D) Cloud native applications must be built as microservices

**2.** A colleague tells you a project is "CNCF Incubating." What has that project demonstrated?

A) It is experimental and not yet widely tested in production
B) It is used successfully in production by a small number of users, with a healthy pool of contributors
C) It is stable, widely adopted, and production ready
D) It has been archived due to inactivity
...
C) In the CNCF TOC's project lifecycle documentation
D) In the CNCF charter
...
B) The list is non-exhaustive
C) A system must use at least four of the seven to be cloud native
D) The list is ordered by importance
...
B) Image immutability is a property of a build artifact; immutable infrastructure is an operational discipline for deployed systems, and the first makes the second practical
C) Immutable infrastructure is the CNCF's term for image immutability
...
- **A is wrong**, and it is wrong in the most instructive possible way. The definition's very first sentence names "public, private, hybrid cloud." Reading *cloud native* as *public cloud* is the single most common misconception about the term, and the definition rules it out twelve words in.
- **B is correct** — verbatim from the definition's second sentence [source: cncf-cloud-native-definition-2026-08-23].
- **C is wrong.** Service meshes appear in the technology list, which the definition explicitly calls non-exhaustive. Nothing in the document requires any specific technology.
- **D is wrong** for the same reason. Microservices are listed as a typical component, not a requirement.

**2. B.** Straight from the projects page: Incubating projects are "used successfully in production by a small number of users with a healthy pool of contributors" [source: cncf-project-maturity-levels-2026-08-23]. **A** is Sandbox — experimental, bleeding edge, not widely tested. **C** is Graduated — stable, widely adopted, production ready. **D** is Archived, which is not a rung on the ladder at all but the terminal state for projects that are no longer maintained.

**3. C.** The projects page describes what the *levels* mean; the criteria for moving between them — due diligence, adopters, governance, security practices — are in the TOC's project lifecycle documentation in the TOC repository [source: cncf-project-maturity-levels-2026-08-23]. **A** is the trap, and it is a reasonable-sounding one: the levels and the criteria feel like they should live together, and they do not. **B** is the Landscape, which catalogs the terrain and has nothing to do with graduation. **D** is the charter, which establishes the bodies and their responsibilities, not the project criteria.

**4. B.** The TOC approves "new projects within the scope of the CNCF set by the Governing Board" [source: cncf-toc-and-tags-2026-08-23]. **A** inverts the relationship — the Board does not work within the TOC's vision; the TOC works within the Board's scope. **C** misassigns the End User TAB, which is the voice of end users in community decisions and feeds requirements to the TOC; it does not approve projects. **D** misassigns TAGs, which coordinate across projects and bridge to the TOC; project approval is the TOC's own vote, and it requires a two-thirds supermajority.

**5. B.** "This list is non-exhaustive" appears in the definition itself, in the same sentence as the list [source: cncf-cloud-native-definition-2026-08-23]. **A** is the misreading the definition preemptively refutes. **C** invents a threshold that appears nowhere. **D** invents an ordering that appears nowhere.

**6. B.** Two different immutabilities. Image immutability *(Ch 2 §2)* is a property of a build artifact: fixed content, addressed by digest. Immutable infrastructure is an operational discipline for deployed systems — infrastructure "that cannot be changed once deployed" [source: cncf-glossary-immutable-infrastructure-2026-08-31] — and the reason it is practical at all is that the artifacts you replace things with are themselves fixed and identifiable.
...
- **C is wrong** — the CNCF glossary carries both concepts under different entries.
...
That grouping is *this book's*, and it is worth being honest about that. Kubernetes does not publish a document titled "the four pluggable interfaces." What Kubernetes publishes is a longer, differently-cut list, and Chapter 10 promised you would see both maps side by side. Here is the other one.
...
Istio, the most widely deployed mesh, states its own version in almost the same words: a service mesh "gives applications capabilities like zero-trust security, observability, and advanced traffic management, without code changes" [source: istio-service-mesh-2026-08-23].
...
Soundings question 4 asked what a sidecar shares with the container beside it. The answer — the Pod's network namespace, meaning `localhost` — is exactly what makes the sidecar mesh model work.

Envoy, the proxy at the heart of Istio's data plane, describes itself as "an L7 proxy and communication bus designed for large modern service oriented architectures" [source: envoy-what-is-envoy-2026-08-31]. And it describes its own deployment shape in a sentence that is worth reading twice:
...
The glossary's framing of *why* is the part worth internalizing:
...
A) It ships configuration flags for each, and the four cases are each configured separately with no common pattern
B) It ships a published interface for each, and in all four cases Kubernetes defines what the thing must do and hands the implementation to somebody else
C) It ships reference implementations that vendors fork and modify, and the four cases are each a fork
D) It ships admission webhooks for each, and all four are implemented as webhook backends

**2.** A team wants to add a new API to their cluster that Kubernetes will store and serve for them, so that `kubectl get` works on their new object type without them running any additional API server. Which extension mechanism fits?

A) The API aggregation layer, registered via an `APIService` object
B) A CustomResourceDefinition
C) A device plugin registered with the kubelet
D) A validating admission webhook
...
A) Yes — NetworkPolicy encrypts traffic between the Pods it allows
B) Yes — the CNI plugin encrypts all pod-to-pod traffic by default
C) No — NetworkPolicy controls which connections are permitted; it does not encrypt them, and nothing in the described setup provides encryption on that leg
D) No — but Ingress TLS termination protects it, since the connection originated over TLS
...
B) The data plane is the proxies that mediate service-to-service communication; the control plane manages and configures those proxies
C) The data plane is the kube-apiserver and etcd; the control plane is the set of Envoy proxies
...
A) Sidecar mode uses Envoy; ambient mode replaces Envoy with a different proxy entirely
B) Ambient mode uses a per-node L4 proxy and optional per-namespace L7 proxies, and the L7 waypoint proxy is a deployment of Envoy — the same engine sidecar mode uses
...
**1. B.** This is the chapter's central claim, and if you got it without looking back, §9 is going to land properly.
...
- **C is wrong.** No forking is involved — that is precisely what a published interface exists to avoid. The whole benefit is that implementations are independent and interchangeable.
- **D is wrong.** Admission webhooks are a real extension point *(Ch 8 §2)*, but they intercept API requests. They do not run containers, wire networks, mount volumes, or define object kinds.

**2. B.** A CustomResourceDefinition is the mechanism for making the kube-apiserver "recognise new kinds of object" [source: k8s-docs-api-aggregation-and-device-plugins-2026-08-31], with the API server handling storage and serving — which is exactly the constraint the question sets.

- **A is the trap**, and a good one. The aggregation layer also adds APIs, but it does so by *proxying to a service you run*. The team explicitly does not want to run another API server, so aggregation is more work than they asked for. The documentation itself flags the distinction: "The aggregation layer is different from Custom Resource Definitions."
- **C is wrong.** Device plugins advertise hardware resources to the kubelet. Different problem, different layer.
- **D is wrong.** A validating webhook can approve or reject objects; it cannot create a new kind.

**3. C.** NetworkPolicy is an allow-list for connectivity. It decides which connections are permitted, using selectors *(Ch 10 §6)*, and encryption is entirely outside its remit *(Ch 10 §7)*.
...
- **B is wrong** as a general claim. Some CNI plugins offer encryption as an additional feature, but nothing in the Kubernetes network model requires it and nothing in the described setup provides it.
- **D is wrong**, and it is the misconception this question exists to catch. TLS *terminated* at the Ingress means the encrypted connection ended there. Whatever continues to the Pod is a new, separate connection — and by default it is plaintext.

This is exactly the gap §5 is about. A service mesh's mTLS is what closes it.

**4. B.** Istio's own framing: the mesh consists of "a data plane (proxies that mediate and control all network communication between services) and a control plane (which manages and configures the proxies)" [source: istio-service-mesh-2026-08-23].

- **A inverts them.** The proxies carry traffic; the control plane configures.
- **C is the vocabulary collision this section exists to defuse.** kube-apiserver and etcd are the *cluster's* control plane *(Ch 3 §2)* — a different structure at a different layer. A mesh's control plane distributes policy to proxies; the cluster's control plane reconciles Kubernetes objects.
- **D is wrong.** They are distinct components with distinct jobs.

**5. B.** In ambient mode "Istio implements its features using a per-node Layer 4 (L4) proxy, and optionally a per-namespace Layer 7 (L7) proxy," and the waypoint proxy "is a deployment of the Envoy proxy; the same engine that Istio uses for its sidecar data plane mode" [source: istio-ambient-mode-2026-08-31].

- **A is wrong** on the key detail. Both modes use Envoy. Ambient adds ztunnel at L4 per node, but the L7 waypoint is Envoy.
...
>
> The serverless property is the **lifecycle** — request-driven, scaled to zero when idle — not the absence of containers or servers.
...
Zero. Not one idle replica burning memory in case somebody shows up. None.
...
>
> "Unlike the HPA, the VPA doesn't come with Kubernetes by default, but is a an add-on that you or a cluster administrator may need to deploy before you can use it" [source: k8s-docs-autoscaling-and-vpa-2026-08-31]. Its own documentation page repeats it: "Unlike HorizontalPodAutoscaler, which is part of the core Kubernetes API, VPA must be installed separately in your cluster" [source: k8s-docs-autoscaling-and-vpa-2026-08-31].
>
...
The VPA also needs metrics-server installed to work at all [source: k8s-docs-autoscaling-and-vpa-2026-08-31], and it has three components worth knowing by function: a **recommender** that analyzes usage and produces recommendations, an **updater** that acts on them, and an **admission controller webhook** that applies recommendations to new or recreated Pods [source: k8s-docs-autoscaling-and-vpa-2026-08-31]. Moves the **per-replica resources**.
...
> ⚓ **Worth Securing:** Karpenter is sponsored by Kubernetes SIG Autoscaling [source: k8s-docs-node-autoscaling-2026-08-31]. It is **not** a CNCF project with a maturity level, and no official source assigns it one. KEDA and Knative are stated as CNCF Graduated in their documentation; Karpenter's is not. If an answer option gives Karpenter a CNCF maturity level, be suspicious of it — and notice that §2 just taught you why that distinction is meaningful.
...
>
> A project whose stated principle is *transparent and accessible* has three closed bodies, and the reason is not hypocrisy — it is that you cannot handle an unreported security vulnerability or a code-of-conduct complaint in a public meeting. Discretion is the requirement, and the exception is deliberate and chartered.
...
A detail that makes the SIG list concrete rather than abstract: several SIGs are ones you have already met the work of. **SIG Network** owns the material of Chapters 9 and 10. **SIG Storage** owns Chapter 11's. **SIG Node** owns much of Chapter 2's and 5's. **SIG Autoscaling** sponsors both node autoscalers from the section you just read. **SIG Release** ships the versions Chapter 8 taught you to reason about [source: k8s-sig-list-and-groups-2026-08-31]. Every interface in §4 has a group of people behind it with a public meeting on a public calendar.
...
> **★ Fixed Point**
>
> **Three minor releases a year** and **three supported minor versions** are not two facts. They are one fact stated twice. Three releases per year × three maintained branches ≈ one year of patch support — and the documentation states that year independently, which means you can *derive* the support window rather than memorize it.
...
This is not a metaphor. The path is written down, and its first rung is genuinely reachable.
...
>
> It does not. SIG Docs exists. So does SIG Contributor Experience. Every SIG has a public meeting on a public calendar, and every one of them has issues nobody has picked up. A great many people on the contributor ladder got their first merged PR by fixing documentation that was wrong, or by writing a test for a code path that had none, or by reproducing a bug report that a maintainer did not have the environment to reproduce.
>
> The reason this matters for an exam chapter, and not just as encouragement: the fastest way to stop finding "SIG, Working Group, Committee, Steering" abstract is to attend one SIG meeting. Forty-five minutes on a video call watching a group of engineers argue about a KEP will do more for your retention of this section than reading it three times. And it is free, public, and on the calendar this week.

The CNCF side has its own on-ramps, and they are distinct from the Kubernetes ones. **LFX Mentorship** is "a mentoring initiative by the Linux Foundation"; **Google Summer of Code** is "a mentoring program for the open source beginners"; **Outreachy** is "a mentoring initiative for the communities traditionally underrepresented in tech" [source: cncf-mentoring-and-community-groups-2026-08-31]. The foundation also "supports the worldwide community of the Cloud Native Community Groups (CNCGs)" [source: cncf-mentoring-and-community-groups-2026-08-31] — free, volunteer-run local meetups, including Kubernetes Community Days.
...
A) The VerticalPodAutoscaler; it moves per-replica resources
B) KEDA; it moves the replica count, on external events and on schedules via its Cron scaler
C) Cluster Autoscaler; it moves the node pool
D) The HorizontalPodAutoscaler; it moves the replica count based on queue depth
...
B) It is an add-on that must be installed separately, and it requires metrics-server to function
C) It adjusts the number of replicas of a workload based on memory pressure
D) It provisions new nodes when Pods cannot be scheduled

**3.** What distinguishes a Kubernetes **Committee** from a SIG and a Working Group?

A) A Committee is longer-lived than a SIG
B) A Committee is scoped to one technical topic; SIGs span topics
C) A Committee does not have open membership and does not always operate in the open; it is formed by the Steering Committee for topics requiring discretion
D) A Committee is a CNCF body; SIGs and Working Groups are Kubernetes bodies
...
A) Approximately three years of support remains, since Kubernetes maintains three years of release branches
B) 1.35 is out of support, because the project maintains only the current release
C) The project maintains release branches for the most recent three minor releases and Kubernetes 1.19 and newer receive approximately one year of patch support; with three releases a year and 1.35 being two behind, it is at or very near the end of that window
D) Support depends entirely on the cloud provider; the upstream project makes no commitment
...
B) They are wrong; Knative builds on the Kubernetes Pod abstraction and Serving and Eventing are implemented as CRDs — the workloads are still containers in Pods, and the serverless property is the lifecycle
C) They are right about Knative Functions but wrong about Knative Serving
D) They are wrong; Knative replaces Kubernetes entirely with its own runtime

---

**Answers with Explanations:**

**1. B.** KEDA scales "based on the number of events to be processed, for example the amount of messages in a queue," and its `Cron` scaler "allows you to define schedules (and time zones) for scaling your workloads in or out" [source: k8s-docs-autoscaling-and-vpa-2026-08-31]. Both requirements, one tool. It moves the replica count — the same axis as the HPA, with a different trigger.
...
- **C is wrong.** Cluster Autoscaler operates on nodes, reacting to unschedulable Pods. It has no view of a queue.
- **D is the near-miss worth understanding.** The HPA scales on "observed resource utilization such as CPU or memory usage" or custom metrics [source: k8s-docs-hpa-2026-08-24]. Getting queue depth in front of an HPA is possible through custom metrics plumbing, but it is not what the HPA does natively, and it does not address the scheduling requirement at all.

**2. B.** "Unlike the HPA, the VPA doesn't come with Kubernetes by default, but is a an add-on that you or a cluster administrator may need to deploy before you can use it," and "You will need to have the Metrics Server installed to your cluster for the VPA to work" [source: k8s-docs-autoscaling-and-vpa-2026-08-31].
...
- **C describes the HPA's axis, not the VPA's.** Vertical means resources per replica.
- **D describes a node autoscaler.**

**3. C.** Committees "do not have open membership and do not always operate in the open. They are formed by the steering committee for specific topics requiring discretion (for example Security, Code of Conduct)" [source: k8s-community-governance-2026-08-23].

- **A is wrong** — longevity is not the distinction. SIGs are the durable unit; Working Groups are the time-bounded one. Committees are distinguished by closed membership.
- **B inverts the SIG/WG relationship.** SIGs are topic-scoped; Working Groups are the ones that cross SIG lines.
- **D confuses the two organizations.** All three — SIGs, Working Groups, Committees — are Kubernetes bodies. CNCF's equivalent coordination units are TAGs.

**4. C.** The project "maintains release branches for the most recent three minor releases," and "Kubernetes 1.19 and newer receive approximately 1 year of patch support" [source: k8s-release-cycle-and-cadence-2026-08-31]. With approximately three releases per year, being two versions behind puts a release at the end of its supported window.

- **A misreads "three releases" as "three years."** The number three attaches to *minor versions*, not years.
- **B understates it — three branches are maintained, not one.**
...
*The same vocabulary as the extension-points map, one altitude higher.*
...
B) Secure, resilient, manageable, sustainable, and observable
C) Declarative, immutable, ephemeral, elastic, and stateless
...
A) The director is right — cloud native by definition requires a public cloud provider
B) The definition's first sentence names "public, private, hybrid cloud," so an on-premises platform can absolutely be cloud native
C) The definition is silent on where workloads run
D) On-premises platforms can be cloud native only if they use a CNCF-certified Kubernetes distribution
...
C) Make high-impact changes frequently and predictably with minimal toil and clear separation of concerns
D) Reduce infrastructure cost by a measurable percentage

**Q4.** A project is described as CNCF Sandbox. Which statement is supported?

A) It is stable, widely adopted, and production ready
B) It is used successfully in production by a small number of users
C) It is experimental, not yet widely tested in production, and on the bleeding edge
D) It has been archived and is no longer supported
...
B) The Governing Board handles business oversight and budget and sets the CNCF's scope; the TOC defines the technical vision and approves projects within that scope
C) Both bodies vote jointly on every project application
D) The Governing Board is elected by the TOC and reports to it
...
A) Nothing — that is the current list
B) TAGs were restructured in 2025; the current TAGs are Developer Experience, Infrastructure, Operational Resilience, Security and Compliance, and Workloads Foundation
...
A) Monoliths always outperform microservices at scale
B) Microservices increase operational overhead, and building a microservices-based app before it has proven valuable may be premature spending of engineering effort
C) Microservices are incompatible with cloud native architecture
D) Monoliths are easier to make immutable
...
A) Kubernetes provides an in-tree volume plugin the vendor configures
B) Kubernetes provides the Container Storage Interface — a published specification — and the vendor writes a CSI driver implementing it
C) The vendor submits a patch to the Kubernetes codebase for each release
D) Kubernetes provides a device plugin framework the vendor registers against

**Q9.** `[retrieval]` A team needs the kube-apiserver to recognize and store a new object kind, served through the standard API so `kubectl get` works, without running any additional API server process. Which mechanism, and which of the four pluggable interfaces does it represent?
...
B) A chart that ships custom resources must install their definitions before the objects that use them, which is an ordering problem the packaging format had to solve explicitly
C) CRDs are cluster-scoped and Helm cannot manage cluster-scoped objects in templates
D) The `crds/` directory is deprecated and exists only for backwards compatibility

**Q11.** What does the Kubernetes documentation say distinguishes the API aggregation layer from CustomResourceDefinitions?

A) They are two names for the same mechanism
B) CRDs make the kube-apiserver recognize new kinds of object; the aggregation layer lets a registered `APIService` claim a URL path and proxies requests to a service you run
C) The aggregation layer is deprecated in favour of CRDs
D) CRDs work only for namespaced resources; aggregation works only for cluster-scoped resources

**Q12.** The defining property of a service mesh, per both the CNCF glossary and Istio's own documentation, is that it adds reliability, observability and security features uniformly across services:

A) With a small, well-documented set of application code changes
B) Without requiring code changes
C) Only for services written in supported languages
D) Only for north-south traffic

**Q13.** `[retrieval: ch10]` A cluster enforces strict default-deny NetworkPolicy and terminates TLS at the Ingress. Which security gap does adding a service mesh with mTLS close?

A) It prevents unauthorized Pods from connecting to a backend Service
B) It encrypts and mutually authenticates service-to-service traffic inside the cluster, including the previously plaintext leg from the Ingress to the Pod
C) It encrypts Secrets stored in etcd
D) It replaces NetworkPolicy, making it unnecessary

**Q14.** `[retrieval: ch12]` A team encrypts service-to-service traffic with mTLS via a mesh, and separately enables encryption at rest for etcd. What do these two measures protect, respectively?

A) Both protect the same data at different times
B) mTLS protects data in transit between workloads; encryption at rest protects the stored contents of the cluster's datastore. Neither substitutes for the other
C) mTLS protects Secrets; encryption at rest protects ConfigMaps
D) Encryption at rest makes mTLS redundant, since traffic originates from encrypted storage

**Q15.** Which statement about Istio's ambient mode is correct?

A) It removes Envoy from the architecture entirely
...
C) It is a competitor product to Istio
D) It requires all Pods in the cluster to migrate away from sidecars simultaneously

**Q16.** Knative Serving and Knative Eventing answer different questions. Which pairing is correct?

A) Serving handles asynchronous events; Eventing handles synchronous HTTP
B) Serving is an HTTP-triggered autoscaling container runtime including scale to zero; Eventing is a CloudEvents-over-HTTP asynchronous routing layer
C) Serving handles ingress routing; Eventing handles logging
D) Serving is for stateful workloads; Eventing is for stateless ones

**Q17.** How does Knative relate to Kubernetes?

A) Knative replaces the Kubernetes control plane with its own scheduler
B) Knative is Kubernetes-based, builds on the Pod abstraction, and Serving and Eventing are implemented as CustomResourceDefinitions
C) Knative runs containers outside of Pods to achieve scale to zero
D) Knative is a fork of Kubernetes maintained separately by the CNCF

**Q18.** `[retrieval: ch13]` A HorizontalPodAutoscaler is created targeting a Deployment. The object is accepted, but the replica count never changes regardless of load. `kubectl top pods` returns an error. What is the most likely cause?

A) The Deployment's selector is misconfigured
B) metrics-server is not installed, so nothing is serving the Metrics API the HPA reads from
C) The HPA requires a VerticalPodAutoscaler to be installed alongside it
D) The HPA control loop runs only once at creation

**Q19.** `[retrieval: ch7]` Pods are stuck in `Pending` because no existing node can satisfy their resource requests. Which component addresses this, and what does it do?

A) The HorizontalPodAutoscaler, by reducing the replica count until the Pods fit
B) The VerticalPodAutoscaler, by lowering the Pods' resource requests
C) A node autoscaler — Cluster Autoscaler or Karpenter — by provisioning new nodes to accommodate unschedulable Pods
D) The scheduler, by preempting lower-priority Pods indefinitely

**Q20.** Which correctly pairs each autoscaler with the axis it moves?

A) HPA → resources per replica; VPA → replica count; Cluster Autoscaler → node pool
B) HPA → replica count; VPA → resources per replica; Cluster Autoscaler → node pool; KEDA → replica count
C) HPA → node pool; KEDA → resources per replica; VPA → replica count
D) All four move the replica count, differing only in what triggers them

**Q21.** `[retrieval: ch8]` The Kubernetes project ships approximately three minor releases per year and maintains release branches for the most recent three minor releases. What follows, and which group runs it?
...
C) Each release is supported until the next one ships; the Steering Committee runs the process
D) Support duration varies by release; no single group is responsible

---

### Answers and Explanations

**Q1 — B.** Verbatim from the definition [source: cncf-cloud-native-definition-2026-08-23]. **A** is a plausible-sounding list of cloud native buzzwords, none of which appear in the characteristics clause. **C** mixes real cloud native concepts — declarative, immutable — with ones the definition does not list as characteristics. **D** is a list of general software quality attributes; "sustainable" and "observable" are the two that distinguish the real answer, and they are the two people most often drop.

**Q2 — B.** "public, private, hybrid cloud" appears twelve words into the definition [source: cncf-cloud-native-definition-2026-08-23]. **A** is the misconception the definition preemptively rules out. **C** is wrong — the definition is explicit, not silent. **D** invents a certification requirement that appears nowhere; conformance certification is a real CNCF program but has nothing to do with whether a platform meets this definition.

**Q3 — C.** The definition's closing clause: techniques combined with robust automation "allow engineers to make high-impact changes frequently and predictably with minimal toil and clear separation of concerns" [source: cncf-cloud-native-definition-2026-08-23]. **A** overclaims — "minimal toil" is not "no operations staff." **B** promises a guarantee no architecture provides. **D** invents a cost claim the definition does not make.

**Q4 — C.** Sandbox projects are "experimental projects not yet widely tested in production, on the bleeding edge of technology" [source: cncf-project-maturity-levels-2026-08-23]. **A** is Graduated. **B** is Incubating. **D** is Archived — a terminal category, not a rung on the ladder.

**Q5 — B.** The Board is "responsible for marketing and other business oversight and budget decisions" [source: cncf-charter-governance-bodies-2026-08-31]; the TOC defines "the technical vision" and approves "new projects within the scope of the CNCF set by the Governing Board" [source: cncf-toc-and-tags-2026-08-23]. **A** inverts the relationship exactly — a common error because "Technical Oversight Committee" sounds more senior than "Board." **C** invents a joint process; project votes are the TOC's, by two-thirds supermajority. **D** invents a reporting line.

**Q6 — B.** The restructuring was approved by the TOC and announced in May 2025 [source: cncf-tags-current-structure-2026-08-31]. **A** is the trap for anyone studying from pre-2025 material. **C** confuses the two organizations — though the confusion has a real historical root, since CNCF's groups were originally called SIGs before being renamed TAGs. **D** is fabricated; the End User TAB is a distinct body with a distinct role.

**Q7 — B.** The glossary states that devolving into microservices "increases its operational overhead," and that "crafting a microservices-based app before it has proven valuable may be premature spending of engineering effort. If the application yields no value, that effort becomes wasted" [source: cncf-glossary-microservices-monoliths-coupling-2026-08-31]. **A** overstates — the argument is about *timing and cost*, not raw performance. **C** contradicts the definition, which lists microservices as a cloud native technology. **D** invents a connection between monoliths and immutability that the glossary does not make.

**Q8 — B.** CSI is a published interface; storage vendors implement drivers against it *(Ch 11 §5)*. **A** describes the pre-CSI in-tree model that CSI migration exists to move away from. **C** is exactly what the interface exists to avoid — the whole point is that vendors ship on their own schedule without touching the Kubernetes codebase. **D** is a real mechanism for a different problem: device plugins advertise hardware resources to the kubelet [source: k8s-docs-api-aggregation-and-device-plugins-2026-08-31], not storage volumes.

**Q9 — B.** A CustomResourceDefinition makes the kube-apiserver recognize new kinds of object, with the API server handling storage and serving [source: k8s-docs-api-aggregation-and-device-plugins-2026-08-31] — and CRDs are the fourth of this book's four pluggable interfaces *(Ch 6 §8)*. **A** names a real mechanism that fails the stated constraint: aggregation proxies to a service you run, so you would be running an additional API server. **C** is not an API-creation mechanism at all. **D** is a scheduling extension.

**Q10 — B.** A chart shipping custom resources must install the CRDs before the objects using them; `crds/` exists to solve that ordering [cross-reference: Ch 14 §6]. **A** is wrong — the constraint is ordering, not security. **C** is wrong; Helm manages cluster-scoped objects routinely. **D** is fabricated.

**Q11 — B.** The documentation states the distinction directly: "The aggregation layer is different from Custom Resource Definitions, which are a way to make the kube-apiserver recognise new kinds of object," and describes aggregation as registering an `APIService` that claims a URL path, after which the layer proxies to the registered service [source: k8s-docs-api-aggregation-and-device-plugins-2026-08-31]. **A** contradicts the documentation. **C** is fabricated — both are current. **D** invents a scope restriction that does not exist.

**Q12 — B.** The CNCF glossary: mesh features are added "uniformly across all services across a cluster without requiring code changes" [source: cncf-glossary-service-mesh-2026-08-31]. Istio: "without code changes" [source: istio-service-mesh-2026-08-23]. **A** negates the defining property. **C** is wrong and is why the sidecar model exists — Envoy's own documentation says it "works with any application language" [source: envoy-what-is-envoy-2026-08-31]. **D** inverts the traffic direction; a mesh's distinctive contribution is east-west, service-to-service.

**Q13 — B.** NetworkPolicy permits and denies connections; it cannot encrypt *(Ch 10 §7)*. TLS terminated at the Ingress ends there. mTLS between workloads closes exactly that gap, with mutual authentication of workload identity [source: istio-security-mtls-identity-2026-08-31]. **A** is what NetworkPolicy already does — the question asks what the mesh *adds*. **C** confuses transit with rest; encryption at rest is a separate control on etcd *(Ch 12 §4)*. **D** overclaims — mesh and NetworkPolicy are complementary controls, and many clusters run both.

**Q14 — B.** Two different data states, two different controls. mTLS protects data in transit; encryption at rest protects the datastore's contents. **A** obscures the distinction that makes both necessary. **C** invents an object-type split that neither mechanism has. **D** is the misconception — data does not stay encrypted because it was encrypted somewhere else; each hop and each resting place needs its own control. This is defense in depth, which Istio names among its own security goals [source: istio-security-mtls-identity-2026-08-31].

**Q15 — B.** Ambient mode "implements its features using a per-node Layer 4 (L4) proxy, and optionally a per-namespace Layer 7 (L7) proxy"; ztunnel is "a purpose-built, per-node proxy"; and the waypoint proxy "is a deployment of the Envoy proxy; the same engine that Istio uses for its sidecar data plane mode" [source: istio-ambient-mode-2026-08-31]. **A** is the trap — ambient removes *sidecars*, not Envoy. **C** is wrong; ambient is an Istio mode. **D** is contradicted directly: sidecar and ambient Pods "can co-exist within the same mesh."

**Q16 — B.** Serving is "an HTTP-triggered autoscaling container runtime… including scale to zero"; Eventing is "a CloudEvents-over-HTTP asynchronous routing layer" [source: knative-overview-2026-08-23]. **A** swaps them, which is the error a reader who knows both names but not their jobs will make. **C** and **D** invent roles for both.

**Q17 — B.** Knative "builds on the Kubernetes Pod abstraction," and Serving and Eventing "are implemented as Kubernetes Custom Resource Definitions (CRDs)" [source: knative-overview-2026-08-23]. **A** and **D** both assert replacement or forking; Knative does neither. **C** is the "serverless means no containers" misconception in mechanical dress — the containers are in Pods, which is exactly what makes scale-to-zero a lifecycle property rather than an architectural one.

**Q18 — B.** The HPA reads from the Metrics API, which metrics-server serves as a cluster addon and reference implementation [source: k8s-docs-resource-metrics-pipeline-2026-08-31]. The failing `kubectl top` is the giveaway — it consumes the same API *(Ch 13 §7)*. **A** would cause different symptoms: the Deployment itself would fail to manage its Pods, not merely fail to scale. **C** is fabricated; the two autoscalers are independent, though both need metrics-server. **D** contradicts the documented behavior — the HPA runs "as a control loop that runs intermittently," by default every 15 seconds [source: k8s-docs-hpa-2026-08-24].

**Q19 — C.** "If there are Pods in a cluster that can't be scheduled on existing Nodes, new Nodes can be automatically added to the cluster — *provisioned* — to accommodate the Pods" [source: k8s-docs-node-autoscaling-2026-08-31]. **A** is backwards — reducing replicas to fit the cluster is not scaling to meet demand. **B** describes something the VPA does not do; it adjusts resources based on usage, not to satisfy the scheduler, and it is an add-on besides. **D** misuses preemption: preemption evicts lower-priority Pods to make room *(Ch 7 §2)*, which relocates the shortage rather than resolving it, and does nothing when there is no lower-priority work to evict.

**Q20 — B.** HPA and KEDA move the replica count on different triggers — utilization versus external events. VPA moves per-replica resources. Cluster Autoscaler and Karpenter move the node pool [source: k8s-docs-autoscaling-and-vpa-2026-08-31], [source: k8s-docs-node-autoscaling-2026-08-31]. **A** swaps horizontal and vertical, the most common error in this material. **C** scrambles all three axes. **D** collapses a genuine distinction — the KEDA/HPA axis overlap is real, but VPA and the node autoscalers move genuinely different things.

**Q21 — B.** "Kubernetes releases currently happen approximately three times per year"; the project "maintains release branches for the most recent three minor releases"; and "Kubernetes 1.19 and newer receive approximately 1 year of patch support" [source: k8s-release-cycle-and-cadence-2026-08-31]. Three per year, three maintained, roughly a year — one fact, three ways. SIG Release owns "production of Kubernetes releases on a reliable schedule" [source: k8s-release-cycle-and-cadence-2026-08-31]. **A** misreads three versions as three years and misassigns the SIG. **C** understates the support window badly — that would be about four months. **D** is wrong on both halves; the project publishes explicit end-of-life dates and a named group runs the process.
...
| **Where criteria live** | The TOC's project lifecycle documentation — not the projects page. Two-week public comment, two-thirds TOC supermajority. |
...
You have finished the largest chapter in this book, and the one that reaches furthest back. Nine chapters fed it. Twenty-six cross-references from earlier chapters pointed at it. Every one of them has now been answered.
...
Domain 4 has one competency left, and it is the one that got demoted.

Under the old five-domain blueprint, Cloud Native Observability was a standalone domain with its own weight. Under the current one it is a competency inside Cloud Native Architecture — which sounds like a downgrade and is not. It sounds like a downgrade because the exam reweighted; it is not one because the question observability answers has only got harder as systems have got more loosely coupled.
...
> — CNCF Cloud Native Glossary, on zero trust architecture. It is a fine principle for a network, and not a bad one for a study plan.